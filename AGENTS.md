...
- `session-resume` (typed reviewer/implementer bootstrap packet built from `ControlPlaneReadModel` and repo artifacts; emits `SessionCachePacket` with `head_sha`, `last_reviewed_sha`, optional frozen `review_candidate`, blockers, interaction mode, and key rules so fresh sessions start from typed state instead of re-reading the repo. It must reuse the same promoted `active_target` / `CoordinationSnapshot` path as `startup-context` and dashboard so stale continuity targets cannot diverge from live plan routing or finding pressure.)
