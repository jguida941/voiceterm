...
| Where is the 2026-05-04 guard-floor and push-preflight coverage closure documented (`check-router --execute --keep-going` emits a typed guard coverage/remediation receipt, phases serial-sensitive projection/status commands away from parallel-safe guards, and the shared bundle floor includes cross-cutting Python/source/registry/runtime-spine guards)? | `dev/scripts/devctl/bundles/registry.py` for the shared guard floor, `dev/scripts/devctl/commands/check/router.py` / `router_execution.py` / `router_render.py` for `CheckRouterGuardCoverageReceipt`, `GuardRemediationAction`, and serial/parallel execution-plan output, `dev/scripts/devctl/governance/push_routing.py` for governed push preflight `--keep-going`, `dev/scripts/devctl/tests/commands/check/test_check_router.py` plus `dev/scripts/devctl/tests/governance/test_bundle_registry.py` for coverage, and `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-04 entry "Check-router now proves guard coverage before publication". |
