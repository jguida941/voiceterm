...
| Where is the bounded typed outcome surface for expired-pending review packets documented? | `dev/scripts/README.md` for `review-channel --action history --include-outcomes` command semantics, `dev/scripts/devctl/review_channel/packet_outcomes.py` for the `PacketOutcomeLedger` model/classifier, and `dev/active/MASTER_PLAN.md` / `rev_pkt_1822` for the remaining full migration + closure-guard scope |
...
| Where is the 2026-04-26 governed-commit attention-staleness closure documented (`devctl commit` refreshes the startup receipt before failing on packet `attention_revision_stale` so typed dogfood finding packets do not invalidate their own publication path)? | `dev/scripts/devctl/commands/vcs/governed_executor_commit_runtime.py` for the shared preflight refresh helper, `dev/scripts/devctl/commands/vcs/governed_executor_stage_attention.py` / `governed_executor_commit_phase.py` for the stage/commit consumers, and `dev/history/ENGINEERING_EVOLUTION.md` 2026-04-26 entry "Governed commit refreshes startup attention before stale gates" |
