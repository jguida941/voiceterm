...
| Where is the 2026-05-04 `/develop` continuation false-blocker closure documented (stopped watcher rows only close when typed packet-attention and `PacketBacklogPressure` evidence are clear; missing pressure evidence fails closed, and idle observer waits with no packet pressure stay current)? | `dev/scripts/devctl/commands/development/continuation_commands.py` for `watcher_report_needed`, `dev/scripts/devctl/commands/development/continuation.py` for continuation reasons, `dev/scripts/devctl/commands/development/orchestration_agent_loop_state.py` for idle observer wait classification, `dev/scripts/devctl/commands/development/report.py` for wiring `packet_pressure` into continuation, `dev/scripts/devctl/tests/commands/test_development_command.py` for regressions, plus `dev/scripts/README.md` and `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-04 entry "`/develop` continuation now gates stopped watchers on typed packet pressure". |
| Where is the 2026-05-04 cross-AI relaunch-loop first slice documented (`SliceClosureEvent` closes a bounded slice, `AgentRelaunchTrigger` queues scheduler-owned relaunch work, quota receipts fail closed, and `devctl relaunch-loop` dogfoods trace -> queue -> dry-run dispatch without using packet delivery as wake authority)? | `dev/scripts/devctl/runtime/relaunch_loop_models.py` for contracts, `dev/scripts/devctl/runtime/relaunch_loop_builder.py` for event/trigger construction, `dev/scripts/devctl/runtime/relaunch_loop_store.py` for queue helpers, `dev/scripts/devctl/commands/governance/relaunch_loop.py` plus `dev/scripts/devctl/cli_parser/relaunch_loop.py` for the controller, `dev/scripts/devctl/platform/runtime_state_contract_rows_relaunch_loop.py` for platform contract registration, `dev/scripts/devctl/tests/runtime/test_relaunch_loop.py` and `dev/scripts/devctl/tests/commands/test_relaunch_loop_command.py` for coverage, plus `dev/scripts/README.md`, `dev/guides/DEVELOPMENT.md`, and `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-04 entry "Relaunch-loop contracts now dogfood slice closure into queued dispatch". |
...
| Where is the 2026-05-04 guard-floor and push-preflight coverage closure documented (`check-router --execute --keep-going` emits a typed guard coverage/remediation receipt, phases serial-sensitive projection/status commands away from parallel-safe guards, applies command/route timeout policy, and the shared bundle floor includes cross-cutting Python/source/registry/runtime-spine guards)? | `dev/scripts/devctl/bundles/registry.py` for the shared guard floor, `dev/scripts/devctl/commands/check/router.py` / `router_execution.py` / `router_render.py` for `CheckRouterGuardCoverageReceipt`, `GuardRemediationAction`, serial/parallel execution-plan output, and `CheckRouterExecutionPolicy`, `dev/scripts/devctl/governance/push_routing.py` for governed push preflight `--keep-going`, `dev/scripts/devctl/tests/commands/check/test_check_router.py` plus `dev/scripts/devctl/tests/governance/test_bundle_registry.py` for coverage, and `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-04 entries "Check-router now proves guard coverage before publication" and "Routed guards now fail visibly on timeout instead of waiting silently". |
| Where is the 2026-05-04 long-running command progress closure documented (`run_cmd` emits typed no-output heartbeats, VCS phases persist `StageProgressEvent`, and `progress-status` gives read-only state instead of manual `ps` polling)? | `dev/scripts/devctl/runtime/stage_progress.py` for `StageProgressEvent` persistence, `dev/scripts/devctl/command_runner.py` / `command_runner_process.py` / `runtime/command_progress.py` for child-process start/heartbeat/complete events, `dev/scripts/devctl/commands/vcs/progress.py` for governed commit/push phase events, `dev/scripts/devctl/commands/reporting/progress_status.py` for the read-only status surface, and `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-04 entry "Long-running devctl stages now emit typed progress heartbeats". |
...
| Where is the 2026-05-03 governed-push managed-projection dirty parser closure documented (`git status --porcelain` output trimmed by the shared git helper must still detect unstaged managed `bridge.md` / `REVIEW_SNAPSHOT.md` drift before any `post_push_green` handoff)? | `dev/scripts/devctl/commands/vcs/push_projection_status.py` for the trimmed-porcelain parser, `push_projection_staging.py` and `push_projection_paths.py` for the split staging/path helpers, `push_projection_receipt.py` for the receipt orchestration, `dev/scripts/devctl/tests/vcs/test_push.py` for the stripped-leading-status regression, plus `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-03 entry "Governed push now detects trimmed managed-projection dirt before green reports" |
...
| Where is the 2026-05-02 launcher-bypass receipt, failure-packet routing, single-agent topology, and empty `/develop` attention closure documented? | `dev/scripts/devctl/commands/review_channel/launcher_discipline_receipts.py` for durable `launcher_discipline_bypassed` events, `dev/scripts/devctl/review_channel/failure_packet_router.py` for bounded safe-auto-apply failure packet routing, `dev/scripts/devctl/commands/vcs/governed_executor_commit_failure_router.py` for routing eligible `vcs.commit` failure envelopes into that shared packet adapter, `dev/scripts/devctl/review_channel/collaboration_session_coordination.py` for explicit `single_agent` topology preservation, `dev/scripts/devctl/commands/development/packet_attention.py` for the empty-attention summary, plus `dev/history/ENGINEERING_EVOLUTION.md` 2026-05-02 entries "Governed commit failures now consume the failure packet router" and "Launcher bypass receipts and failure routing become typed audit evidence" |
...
7. **When host process access is available**, run:
   ...
   wrappers intentionally reparent to PID 1, but strict audit must still
   compare any embedded `SessionCachePacket.head_sha` / review-state
   `head_sha` against current `HEAD`. Registered conductors with stale resume
   authority become `stale_supervised_conductors` and `process-cleanup
   --verify` must retire the full conductor tree; unregistered detached
   helpers still fail strict audit/verify.
...
- `process-audit` (host-side Activity Monitor equivalent for repo-related runtime/tooling process trees; reports matched roots plus descendants, includes repo-cwd runtime/tooling helpers that would otherwise look generic in Activity Monitor, fails fast if `ps` is unavailable, preserves registered review-channel conductors as supervised only while their embedded resume/review-state `head_sha` matches current `HEAD`, and `--strict` turns leftover runtime/test trees or stale/orphaned repo-related helpers into a blocking failure before handoff)
...
| `python3 dev/scripts/devctl.py check-router --since-ref origin/develop --execute --keep-going` | before push when scope spans docs/runtime/tooling/release surfaces | auto-selects the stricter required lane, includes risk add-ons, phases serial-sensitive projection/status commands away from parallel-safe guards, applies command/route timeout policy, and reports typed guard coverage/remediation evidence; use `--parallel-workers <n>` to tune, `--no-parallel` to force sequential routing, or `--command-timeout-seconds` / `--route-timeout-seconds` when measured evidence justifies a different budget |
| `python3 dev/scripts/devctl.py progress-status --format md` | a long `devctl`/guard/preflight run looks quiet or you need current command evidence | reads typed `StageProgressEvent` artifacts for the latest command start, heartbeat, completion, or governed VCS phase without spawning agents or waking packet handlers |
...
| `python3 dev/scripts/devctl.py process-cleanup --verify --format md` | after PTY/runtime tests, manual tooling bundles, or before handoff when host access is available | safely kills orphaned/stale repo-related host process trees, including descendant PTY children, repo-cwd background helpers, orphaned tooling descendants, and stale supervised review-channel conductors, then reruns strict host audit |
...
and it validates agent-loop decisions only for live rows that still have loop
authority; stale read-only visibility rows do not create wake/attention work.
