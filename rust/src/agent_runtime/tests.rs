//! Regression tests for agent CLI execution, cancellation, and event delivery.

use super::backend::{
    BoundedEventQueue, CancelToken, CodexCallError, CodexEvent, CodexEventKind, CodexJob,
    CodexJobStats, EventSender,
};
use super::cli::{
...
#[test]
fn backend_threads_return_to_baseline() {
    let baseline_threads = super::active_backend_threads();
    let backend = CodexCliBackend::new(crate::config::AppConfig::parse_from(["test"]));
    let (mut job, hook_guard) = super::with_job_hook(
        Box::new(|_, cancel| {
            assert!(!cancel.is_cancelled());
            vec![CodexEventKind::Finished {
                lines: vec!["done".to_string()],
                status: "ok".to_string(),
                stats: CodexJobStats::new(Instant::now()),
            }]
        }),
        || {
            backend
                .start(CodexRequest::chat("memory-guard".to_string()))
                .expect("backend job should start")
        },
    );
    job.take_handle()
        .expect("backend thread handle")
        .join()
        .expect("backend thread should finish");
    drop(hook_guard);

    for _ in 0..40 {
        if super::active_backend_threads() <= baseline_threads {
            return;
        }
        thread::sleep(Duration::from_millis(5));
    }
    assert!(
        super::active_backend_threads() <= baseline_threads,
        "backend thread count did not return to baseline"
    );
}
