...
pub(in crate::agent_runtime) fn normalize_control_bytes(raw: &[u8]) -> Vec<u8> {
...
pub(in crate::agent_runtime) fn init_guard(len: usize) -> usize {
    len.saturating_mul(4).max(16)
}

pub(in crate::agent_runtime) fn step_guard(guard: &mut usize) -> bool {
    if *guard == 0 {
        return false;
    }
    *guard -= 1;
...
pub(in crate::agent_runtime) fn pop_last_codepoint(buf: &mut Vec<u8>) -> bool {
    if buf.is_empty() {
        return false;
    }
    if buf.last() == Some(&b'\n') {
...
pub(in crate::agent_runtime) fn current_line_start(buf: &[u8]) -> usize {
    buf.iter()
        .rposition(|&b| b == b'\n')
        .map(|pos| pos + 1)
        .unwrap_or(0)
...
pub(in crate::agent_runtime) fn clamp_line_start(line_start: usize, buf: &[u8]) -> usize {
...
pub(in crate::agent_runtime) fn skip_osc_sequence(bytes: &[u8], mut cursor: usize) -> usize {
...
pub(in crate::agent_runtime) fn find_csi_sequence(
    bytes: &[u8],
    start: usize,
) -> Option<(usize, u8)> {
