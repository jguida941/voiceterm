...
pub(in crate::agent_runtime) fn duration_ms(duration: Duration) -> f64 {
    duration.as_secs_f64() * 1000.0
}

pub(in crate::agent_runtime) fn compute_deadline(start: Instant, timeout: Duration) -> Instant {
    start + timeout
}

pub(in crate::agent_runtime) fn should_accept_printable(
    has_printable: bool,
...
pub(in crate::agent_runtime) fn should_fail_control_only(
    has_printable: bool,
...
pub(in crate::agent_runtime) fn should_break_overall(
    elapsed: Duration,
...
pub(in crate::agent_runtime) fn first_output_timed_out(now: Instant, deadline: Instant) -> bool {
    now >= deadline
}

#[derive(Default)]
pub(in crate::agent_runtime) struct SanitizedOutputCache {
    text: String,
    dirty: bool,
    #[cfg(test)]
    refresh_count: usize,
...
impl SanitizedOutputCache {
    pub(in crate::agent_runtime) fn mark_dirty(&mut self) {
        self.dirty = true;
    }

    pub(in crate::agent_runtime) fn sanitized<'a>(&'a mut self, raw: &[u8]) -> &'a str {
        if self.dirty {
    ...
    #[cfg(test)]
    pub(in crate::agent_runtime) fn refresh_count(&self) -> usize {
        self.refresh_count
    }
}

pub(in crate::agent_runtime) trait CodexSession {
...
pub(in crate::agent_runtime) fn call_codex_via_session<S: CodexSession>(
    session: &mut S,
    prompt: &str,
    cancel: &CancelToken,
