//! Core agent request/event model that enforces bounded queues under load.
...
impl BoundedEventQueue {
    ...
    pub(super) fn push(&self, event: CodexEvent) -> Result<(), BackendQueueError> {
        let mut queue = lock_or_recover(&self.inner, "agent_runtime::BoundedEventQueue::push");
    ...
    pub(super) fn drain(&self) -> Vec<CodexEvent> {
        let mut queue = lock_or_recover(&self.inner, "agent_runtime::BoundedEventQueue::drain");
        queue.drain(..).collect()
    }
