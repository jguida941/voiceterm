...
def collect_warning_lint_counts(lines: Iterable[str]) -> dict[str, int]:
    """Collect warning counts keyed by lint code."""
...
def build_clippy_command(*, deny_warnings: bool, extra_args: Iterable[str] = ()) -> list[str]:
    """Build the cargo clippy command used for JSON lint collection."""
    command = ["cargo", "clippy", "--workspace", "--all-features", "--message-format=json"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect clippy warning count and status for CI badge updates.")
    parser.add_argument("--working-directory", default="rust", help="Directory where cargo clippy should run")
    parser.add_argument("--output-json", default="", help="Optional path for summary JSON output")
    parser.add_argument("--github-output", default="", help="Optional path to append GitHub Actions step outputs")
    parser.add_argument("--output-lints-json", default="", help="Optional path for lint-code histogram JSON output")
    parser.add_argument("--deny-warnings", action="store_true", help="Run clippy with `-D warnings`")
    parser.add_argument(
        "--extra-clippy-arg",
        action="append",
        default=[],
        help="Extra argument forwarded after `cargo clippy --`; repeat for multiple args.",
    )
    parser.add_argument("--quiet-json-stream", action="store_true", help="Do not stream JSON compiler output to stdout")
    parser.add_argument(
        "--propagate-exit-code",
        action="store_true",
    ...
    try:
        ...
        lines = []
        exit_code = 127
        command = build_clippy_command(deny_warnings=args.deny_warnings, extra_args=args.extra_clippy_arg)

    warning_count = count_warning_messages(lines)
    lint_counts = collect_warning_lint_counts(lines)
    summary = build_summary(exit_code, warning_count, command=command, working_directory=str(working_directory))
