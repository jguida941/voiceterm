#!/usr/bin/env python3
"""Ralph AI fix wrapper — reads CodeRabbit backlog, invokes Claude Code
for remediation, validates via architecture checks, and commits results.

Environment: RALPH_ATTEMPT, RALPH_REPO, RALPH_BRANCH, RALPH_BACKLOG_DIR,
RALPH_BACKLOG_COUNT, RALPH_SOURCE_RUN_ID, RALPH_SOURCE_SHA, RALPH_APPROVAL_MODE.
Exit codes: 0 = success/no-op, 1 = fix failed, 2 = config error.
"""
...
from dev.scripts.devctl.approval_mode import (  # noqa: E402
    DEFAULT_APPROVAL_MODE,
    normalize_approval_mode,
)
from dev.scripts.devctl.ralph_guardrail_report import (  # noqa: E402
    build_guardrail_report,
    load_guardrails_config,
    render_report_json,
)

# Architecture-specific check commands run after fixes to validate
ARCH_CHECKS: dict[str, list[list[str]]] = {
    ...
    "python-devctl": [
        ["python3", "-m", "pytest", "dev/scripts/devctl/tests/", "-q", "--tb=short"],
    ],
    "python-operator-console": [
        ["python3", "-m", "pytest", "app/operator_console/tests/", "-q", "--tb=short"],
    ],
    "ios": [
        # iOS checks are build-only in CI; simulator tests require macOS runner
        ["python3", "dev/scripts/devctl.py", "mobile-status", "--format", "json"],
    ],
}

# Fallback category-to-architecture map (used when guardrails.json is unavailable)
...
def _build_standards_context(
    ...
    if not standards:
        return ""
    # Collect unique categories from findings to show relevant standards
...
def build_prompt(
    ...
    findings_text = "\n".join(
        f"  {i+1}. [{item.get('severity', 'unknown')}] "
...
def commit_and_push(branch: str, attempt: int, item_count: int) -> bool:
    ...
    # Stage all modified/deleted files (not untracked to avoid accidents)
...
def _build_fix_results(
    items: list[dict], changes_made: bool, checks_passed: bool,
) -> list[dict]:
    ...
    if not changes_made:
        ...
        status = "pending"
    return [
        {"summary": str(it.get("summary", "")), "status": status, "fix_skill": ""}
        for it in items
    ]
...
def main() -> int:
    ...
    print(f"[ralph-ai-fix] attempt={attempt} repo={repo} branch={branch} "
          f"backlog_count={backlog_count}")

    # Load guardrails config for standards context
    ...
    # Build prompt (with standards context) and invoke AI
    ...
    if not changes_made:
        ...
        report = build_guardrail_report(
            items, fix_results, guardrails_config, attempt, repo, branch,
        )
    ...
    # Build and emit guardrail report regardless of check outcome
