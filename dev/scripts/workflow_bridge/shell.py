...
CLI_SCHEMA_PATHS = frozenset({"rust/src/bin/voiceterm/config/cli.rs", "rust/src/config/mod.rs"})
...
def resolve_range(args: argparse.Namespace) -> int:
    ...
    if diff.returncode != 0:
        print(f"failed to compute git diff range {since_ref}..{head_ref}: {diff.stderr.strip()}", file=sys.stderr)
    ...
    _append_output(
        args.github_output,
        [("since", since_ref), ("head", head_ref), ("changed_files_count", str(len(changed_files)))],
    )
    return 0
...
def resolve_security_scope(args: argparse.Namespace) -> int:
    ...
    _append_output(args.github_output, [("since_ref", since_ref or ""), ("head_ref", head_ref or "")])
    return 0
...
def prepare_failure_artifact_dir(args: argparse.Namespace) -> int:
    ...
    _append_output(args.github_output, [("artifact_dir", artifact_dir.as_posix()), ("workflow_slug", workflow_slug)])
    return 0
...
def find_first_file(args: argparse.Namespace) -> int:
    ...
    if not output_value and not args.allow_missing:
        print(f"no files found matching pattern '{args.pattern}' under {search_root}", file=sys.stderr)
        return 1
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    resolve_range_parser = sub.add_parser("resolve-range", help="Resolve safe git range refs and emit changed files.")
    ...
    resolve_range_parser.add_argument("--changed-files-output", required=True, type=Path)
    ...
    find_file_parser = sub.add_parser("find-first-file", help="Find the first lexicographically-sorted matching file.")
    ...
    coverage_parser = sub.add_parser("print-coverage-summary", help="Print a compact Cobertura XML coverage summary.")
