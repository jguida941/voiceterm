...
def _build_parser() -> argparse.ArgumentParser:
    ...
    resolve = subcommands.add_parser("resolve-config", help="Resolve and validate workflow config, then append outputs.")
    resolve.add_argument("--github-output", required=True)
    resolve.add_argument("--event-name", default="", help="Optional explicit event name override (defaults to EVENT_NAME env var).")

    extract = subcommands.add_parser("extract-failure-reason", help="Print the stripped `reason` field from a loop JSON payload.")
    extract.add_argument("--input", required=True)

    run_loop = subcommands.add_parser("run-loop", help="Execute the mutation loop command with workflow soft-fail policy.")
