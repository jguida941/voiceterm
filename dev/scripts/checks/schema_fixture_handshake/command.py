...
def evaluate_schema_fixture_handshake(
    *,
    ...
    for row in rows:
        ...
        valid_files = tuple(
            fixture_path
            for fixture_path in _json_files(valid_dir)
            if _fixture_targets_row(row, fixture_path, expected_role="valid")
        )
        invalid_files = tuple(
            fixture_path
            for fixture_path in _json_files(invalid_dir)
            if _fixture_targets_row(row, fixture_path, expected_role="invalid")
        )
        if not valid_files:
            violations.append(
                _violation(
...
def _fixture_targets_row(
    ...
    *,
    ...
    expected_result = "accept" if expected_role == "valid" else "reject"
    return (
        payload.get("contract_id") == row.registered_contract_id
        and payload.get("entry_kind") == row.entry_kind
        and payload.get("fixture_role") == expected_role
        and payload.get("expected_result") == expected_result
    )
