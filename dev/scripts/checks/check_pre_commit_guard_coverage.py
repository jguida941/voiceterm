...
REQUIRED_HOOK_GUARDS: tuple[HookGuardRequirement, ...] = (
    ...
    HookGuardRequirement(
        ...
        required_now=False,
        rationale="G8 requires this once the current-plan authority guard is promoted.",
    ),
    HookGuardRequirement(
        guard_id="check_orphan_files",
        required_text="check_orphan_files.py",
        required_now=False,
        rationale="G2 placeholder until the orphan-file guard is implemented.",
    ),
    HookGuardRequirement(
        guard_id="check_feature_completion",
        required_text="check_feature_completion.py",
        required_now=False,
        rationale="G3 placeholder until the feature-completion guard is implemented.",
    ),
    HookGuardRequirement(
        guard_id="check_plan_row_must_advance",
        required_text="check_plan_row_must_advance.py",
        required_now=False,
        rationale="G4 placeholder until the plan-row advancement guard is implemented.",
    ),
    HookGuardRequirement(
        guard_id="check_no_ingestion_churn_without_advancement",
        required_text="check_no_ingestion_churn_without_advancement.py",
        required_now=False,
        rationale="G5 placeholder until the ingestion-churn guard is implemented.",
    ),
)
