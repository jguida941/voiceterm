...
ENFORCEMENT_EXEMPTIONS = {
    ...
    "packet_absorption_required": {
        "kind": "stateful-backlog-guard",
        "reason": "operates on live review-state packet debt; baseline remediation is tracked before promotion to default release gating",
    },
}
...
def build_report(repo_root: Path = REPO_ROOT) -> dict:
    ...
    for script_kind, script_id, relative_path in tracked_scripts:
        ...
        internal_pipeline_refs = list(INTERNAL_PIPELINE_SCRIPT_REFS.get(script_id, ()))
        enforced = bool(
            direct_bundle_refs
            or direct_workflow_refs
            or indirect_bundle_refs
            or indirect_workflow_refs
            or internal_pipeline_refs
        )
        entry = {
            "kind": script_kind,
            "script_id": script_id,
            ...
            "internal_pipeline_refs": internal_pipeline_refs,
            "exemption": exemption,
            "enforced": enforced,
        }
