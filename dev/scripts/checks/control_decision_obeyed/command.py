#!/usr/bin/env python3
"""Fail when the next attempted action violates a controller decision."""
...
def render_markdown(report: Mapping[str, object]) -> str:
    lines = ["# check_control_decision_obeyed", ""]
    lines.append(f"- ok: {report.get('ok')}")
    lines.append(f"- decision_present: {report.get('decision_present')}")
    lines.append(f"- attempted_action_count: {report.get('attempted_action_count')}")
