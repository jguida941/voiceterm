...
build_function_exception_key = _ia("code_shape_policy", "build_function_exception_key")
...
list_changed_paths_with_base_map = _ia("git_change_paths", "list_changed_paths_with_base_map")
...
def _evaluate_function_shape(*, path, policy, policy_source, text, today):
    return evaluate_function_shape_impl(
        ...
        exception_key_builder=build_function_exception_key,
    )
...
def main() -> int:
    ...
    try:
        if args.absolute:
            ...
            base_map = {path: path for path in changed_paths}
        else:
            if args.since_ref:
    ...
    for path in changed_paths:
        ...
        if args.absolute:
            ...
            if args.since_ref:
                ...
                base_lines = _count_lines(guard.read_text_from_ref(base_path, "HEAD"))
                current_text = guard.read_text_from_worktree(path)
                current_lines = _count_lines(current_text)
