...
def build_instruction_boot_card(
    *,
    ...
    lines = [
        ...
        "- Direct Codex bootstrap `codex --dangerously-bypass-approvals-and-sandbox review --uncommitted` is review-only recovery over uncommitted state when launcher authority ordering is broken; it is not staging, commit, push, release, or proof-of-correctness authority.",
        ...
        "- Governed push preflight carries typed `ValidationScope`: standalone checks default to `live_worktree`, while authorized publication phases use `pipeline_authorized_phase` so live projection guards remain visible evidence without hard-blocking the already-authorized commit range; range-scoped release bundles route publication-scope checks through the push adapter so no-upstream extraction branches do not fall back to `@{u}`.",
        ...
        "- Release hygiene requires stable check-script entrypoints to be documented in `dev/scripts/README.md`; do not publish a guard shim that is invisible to the maintainer script inventory.",
        ...
        "- Guard-enforcement inventory treats shared guards, governed internal pipeline lanes, explicit contextual-input exemptions, governance-closure CI smoke coverage, and root-scoped package-layout ratchets as separate states; do not run no-subject contextual guards as release commands just to satisfy catalog coverage, and use `--stdin --allow-empty` only for CI smoke while live proof still requires typed input, receipts, or the governed push adapter.",
        ...
        "- `FeatureProofReceipt.real_life_test_status=proven_passed` requires a concrete pytest node id selected by the receipt builder; validation bundles, guard actions, projection refreshes, or plan refs alone must remain `not_tested_with_rationale`.",
