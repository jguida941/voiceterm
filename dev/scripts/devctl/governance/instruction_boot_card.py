...
def build_instruction_boot_card(
    *,
    ...
    lines = [
        ...
        "- Use `python3 dev/scripts/devctl.py test-python --suite devctl --path <test>` for focused Python proof instead of raw broad pytest; multi-target devctl proof is split into serial single-target sessions with the typed timeout selected by `check-router`.",
