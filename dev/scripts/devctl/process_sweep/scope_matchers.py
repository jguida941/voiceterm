...
def match_review_channel_conductor_rows(rows: list[dict]) -> list[dict]:
    matched_pids = {
        row["pid"]
        for row in rows
        if REVIEW_CHANNEL_CONDUCTOR_SCRIPT_RE.search(row["command"])
        or REVIEW_CHANNEL_CONDUCTOR_PROMPT_RE.search(row["command"])
    }
    return select_matching_rows(rows, matched_pids, scope="review_channel_conductor")
