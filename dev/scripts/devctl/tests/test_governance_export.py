...
class GovernanceExportSupportTests(unittest.TestCase):
    ...
    def test_build_export_copies_sources_and_creates_zip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            (repo_root / "dev" / "scripts" / "devctl").mkdir(parents=True, exist_ok=True)
            ...
            self.assertTrue((snapshot_dir / "dev" / "scripts" / "checks" / "probe_one.py").exists())
            self.assertTrue((snapshot_dir / "generated" / "snapshot_manifest.json").exists())
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch(
                ...
                args = cli.build_parser().parse_args(
                    ["governance-export", "--format", "json", "--output", str(output_path)]
                )
