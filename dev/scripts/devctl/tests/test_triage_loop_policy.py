"""Unit tests for triage-loop fix-command policy gating."""
...
class TriageLoopPolicyTests(unittest.TestCase):
    ...
    def test_autonomy_mode_operate_allows(self) -> None:
    ...
    def test_allowed_prefix_matches(self) -> None:
        with patch.dict(os.environ, {"AUTONOMY_MODE": "operate"}, clear=False):
            reason = evaluate_fix_policy(
                mode="fix-only",
                branch="develop",
                fix_command="python3 dev/scripts/devctl.py check --profile ci --no-parallel",
                policy=_policy(),
            )
        self.assertIsNone(reason)
    ...
    def test_env_override_prefixes(self) -> None:
        env = {
            ...
            "TRIAGE_LOOP_ALLOWED_PREFIXES": "bash -lc",
        }
