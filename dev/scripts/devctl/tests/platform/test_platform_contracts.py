...
def test_platform_blueprint_contract_shapes_cover_lifecycle_and_authority() -> None:
    ...
    assert "role_review_receipt_refs" in contract_map["FeatureProofReceipt"]
    assert "role_review_terminal_refs_present" in contract_map["NonTrivialOutputProof"]
    ...
    assert "role_review_lifecycles" in contract_map["RemoteCommitPipelineContract"]
