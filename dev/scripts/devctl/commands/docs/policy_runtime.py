...
def resolve_docs_check_policy(
    *,
    ...
    evolution_doc = str(section.get("evolution_doc") or EVOLUTION_DOC).strip()
    if not evolution_doc:
        evolution_doc = EVOLUTION_DOC
    return DocsCheckPolicy(
        user_docs=_coerce_string_list(section.get("user_docs"), USER_DOCS),
...
def is_tooling_change(path: str, *, policy_path: str | None = None) -> bool:
    """Return True when the changed path is considered tooling/process scope."""
    policy = resolve_docs_check_policy(policy_path=policy_path)
