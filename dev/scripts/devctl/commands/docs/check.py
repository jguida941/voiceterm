"""Topical docs-check command implementation."""
...
_should_run_strict_release_user_docs_gate = should_run_strict_release_user_docs_gate
_scan_deprecated_references = partial(scan_deprecated_references, REPO_ROOT)
_run_active_plan_sync_gate = partial(
    run_json_policy_gate,
    ACTIVE_PLAN_SYNC_SCRIPT,
    "active-plan sync gate",
)
_run_multi_agent_sync_gate = partial(
    run_json_policy_gate,
    MULTI_AGENT_SYNC_SCRIPT,
    "multi-agent sync gate",
)
_run_markdown_metadata_header_gate = partial(
    run_json_policy_gate,
    MARKDOWN_METADATA_HEADER_SCRIPT,
    "markdown metadata header gate",
)
_run_workflow_shell_hygiene_gate = partial(
    run_json_policy_gate,
    WORKFLOW_SHELL_HYGIENE_SCRIPT,
    "workflow shell hygiene gate",
)
_run_bundle_workflow_parity_gate = partial(
    run_json_policy_gate,
    BUNDLE_WORKFLOW_PARITY_SCRIPT,
    "bundle/workflow parity gate",
)
_run_agents_bundle_render_gate = partial(
    run_json_policy_gate,
    AGENTS_BUNDLE_RENDER_SCRIPT,
    "AGENTS bundle render gate",
)
_run_guide_contract_sync_gate = partial(
    run_json_policy_gate,
    GUIDE_CONTRACT_SYNC_SCRIPT,
    "guide contract sync gate",
)
_run_instruction_surface_sync_gate = partial(
    run_json_policy_gate,
...
def _emit_docs_check_report(args, report: dict) -> int:
    """Render and emit the final docs-check report payload."""
    output = (
...
def run(args) -> int:
    ...
    if getattr(args, "strict_release", False):
        effective_strict_user_docs = True
        effective_user_facing = effective_user_facing and _should_run_strict_release_user_docs_gate(
            changed=changed,
            user_docs=user_docs,
            head_ref=head_ref,
    ...
    evaluation = evaluate_docs_state(
        DocsEvaluationInput(
            ...
            strict=effective_strict_user_docs,
    ...
    failure_reasons = build_failure_reasons(
        ...
        strict_user_docs=effective_strict_user_docs,
    ...
    report["strict_release"] = bool(getattr(args, "strict_release", False))
    report["effective_user_facing"] = effective_user_facing
    report["effective_strict_user_docs"] = effective_strict_user_docs
    return _emit_docs_check_report(args, report)
