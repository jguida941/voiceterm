...
from .process_sweep_scope_matchers import (
...
def lookup_process_cwds(rows: list[dict], *, run_cmd: RunCmd) -> tuple[dict[int, str], list[str]]:
...
def scan_matching_processes(
    ...
    *,
    skip_pid: int | None,
    scope: str,
    repo_cwd_candidate_regex: re.Pattern[str] | None,
    run_cmd: RunCmd,
    current_pid: int,
    parent_pid: int,
) -> tuple[list[dict], list[str]]:
