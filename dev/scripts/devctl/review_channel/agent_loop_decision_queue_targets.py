...
def _packet_targets_fresh_route(
...
def _route_freshness(row: Mapping[str, object]) -> str:
    ...
    idle_seconds = _coerce_count(row.get("idle_seconds"))
    stale_after = _coerce_count(row.get("stale_after_seconds"))
