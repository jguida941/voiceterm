...
def _render_start_rules_body(
    *,
    ...
    lines = [
        ...
        f"`{implementer_capability.startup_context_command}` first.",
        f"   If {implementer_name}'s receipt exits non-zero, checkpoint or repair the",
        "   repo state before coding or relaunching conductor work.",
        f"   If {reviewer_name}'s receipt exits non-zero, read the summary fields",
