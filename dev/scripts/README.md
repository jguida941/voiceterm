...
| `dev/scripts/checks/check_command_output_consumed.py` | Command-output-consumed shim | Backward-compatible entrypoint for the packaged `command_output_consumed.command` guard (control-plane command outputs must be consumed by a typed reducer). |
| `dev/scripts/checks/check_control_decision_consistency.py` | Control-decision consistency shim | Backward-compatible entrypoint for the packaged `control_decision_consistency.command` guard (typed control decisions stay internally consistent across surfaces). |
| `dev/scripts/checks/check_control_decision_obeyed.py` | Control-decision obeyed shim | Backward-compatible entrypoint for the packaged `control_decision_obeyed.command` guard (emitted control decisions are actually executed, not dropped). |
| `dev/scripts/checks/check_packet_absorption_required.py` | Packet-absorption shim | Backward-compatible entrypoint for the packaged `packet_absorption_required.command` guard (review-channel packets are absorbed into typed state before closure). |
| `dev/scripts/checks/check_publication_scope_integrity.py` | Publication scope integrity guard | Fails closed when a publication candidate is contaminated by dirty worktree state outside the declared publication scope. |
| `dev/scripts/checks/check_substrate_commits_have_applied_plan_row.py` | Substrate-commit plan-row coverage guard | Requires substrate commits to be covered by applied typed PlanRows so infrastructure changes retain plan provenance. |
