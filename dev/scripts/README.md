...
- `process-cleanup`: host-side cleanup for orphaned/stale repo-related process trees; expands cleanup roots to full descendant trees so leaked PTY children, repo-cwd background helpers, and orphaned tooling descendants are reaped with their parent wrappers when possible, skips recent active processes by default, treats registry-backed running review-channel conductor PIDs as protected even when prepared authority is stale, and `--verify` reruns strict host audit after cleanup
