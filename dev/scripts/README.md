...
| `dev/scripts/checks/check_semantic_tdd_evidence_log.py` | Semantic-TDD evidence-log forcing function | Warns when semantic-TDD canonical surfaces (`test_live_state_invariants.py`, `semantic_tdd_role.py`, topology/reviewer/authority runtime files) change without a paired `evidence.md` case update. Fail-OPEN by default (`--mode warn` exits 0 + visible warning); `--mode strict` fails closed so CI/pre-commit can opt-in. Scans `--scope staged` (default), `--scope last_commit`, or `--scope both`. Supports `--format json|md`. The forcing-function exists because per-slice receipt discipline tends to decay; the guard makes the gap mechanical instead of cultural. |
...
| `dev/scripts/checks/check_no_prose_authority_promotion.py` | Prose-authority promotion guard | Scans hand-maintained docs (`dev/guides/DEVELOPMENT.md`, `dev/guides/SYSTEM_MAP.md`, `dev/active/INDEX.md`) for prose that promotes generated boot cards or active markdown above typed state. Combines a legacy substring scan over explicit disallowed phrases with a contextual scan that flags protected-doc references co-occurring with dangerous authority terms (e.g. `source of truth`, `canonical tracker`, `canonical registry`) unless the line is qualified as a projection, pointer index, or historical changelog. Supports `--format`. |
...
| `dev/scripts/checks/check_commit_complete_proof.py` | Commit mutation proof guard | Verifies selected commit SHAs or a git range have verified `GitMutationProofReceipt(commit)` rows before commit-complete projections can be treated as durable proof. Supports `--since-ref`, `--head-ref`, `--require-non-empty-range`, and `--format`. |
| `dev/scripts/checks/check_push_complete_proof.py` | Push mutation proof guard | Verifies an explicit pushed SHA claim has a matching verified `GitMutationProofReceipt(push)` row for the claimed remote and branch; without an expected SHA claim it reports a no-op warning instead of blocking generic live-worktree checks. Supports `--remote`, `--branch`, `--expected-sha`, `--allow-live-head-default`, and `--format`. |
| `dev/scripts/checks/check_no_projection_proof_misuse.py` | Projection proof-misuse guard | Rejects `commit.complete` progress projections that lack verified commit mutation proof, using the repo-policy legacy baseline by default and `--ignore-before-line` for explicit debugging overrides. Supports `--progress-path` and `--format`. |
...
| `dev/scripts/checks/check_multi_agent_sync.py` | Multi-agent coordination gate | Verifies `MASTER_PLAN` board parity with the merged markdown-swarm tables in `dev/active/review_channel.md` for dynamic `AGENT-<N>` lanes (lane/MP/worktree/branch alignment, instruction/ack protocol checks, lane-lock + MP-collision handoff checks, status/date formatting, ledger traceability, and required end-of-cycle signoff when all agent lanes are merged). Its packaged runtime-truth helpers also distinguish scoped communication-only packet-body focus from mutation authority so inbox projections do not override stricter typed body-open work, and allow active packet-ingestion lifecycle focus to supersede the inbox top item only while that active packet remains in `pending_actionable_packet_ids`. |
