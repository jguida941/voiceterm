...
- `session-resume`: compact cached role bootstrap packet over the same typed
  ...
  falls back to commit-range review only when no valid candidate exists. It
  now shares the same promoted `active_target` / `CoordinationSnapshot` path
  as `startup-context` and dashboard so live findings can outrank stale
  continuity.
...
- `agent-mind --since-cursor`: cursor-based cross-agent polling over rollout
  ...
  first truncating to a fixed raw-line tail window; this keeps decisions
