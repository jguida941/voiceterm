...
- `startup-context` is the typed startup packet for AI sessions. It composes
  ...
  `allowed_actions`, `blocked_actions`, `recovery_action`,
  ...
  it from partial state. Startup now also carries a bounded
...
- `agent-mind --since-cursor`: cursor-based cross-agent polling over rollout
  ...
  visible even after more than 400 intervening low-signal noise events.
