...
| `review-channel --action status --terminal none --format md` | you need the current bridge-backed review snapshot without relaunching anything | reads `dev/active/review_channel.md` + `bridge.md`, refreshes `dev/reports/review_channel/latest/`, and emits the current lane/bridge projection set for operator or tooling consumers |
