...
- `startup-context`: typed startup packet for AI agent sessions; composes repo governance, reviewer gate, and push/checkpoint advisory state, prefers typed review-state acceptance when present, falls back to bridge compatibility reads only when the typed projection is unavailable, and renders continuity roots only when the canonical memory/context directories actually exist
