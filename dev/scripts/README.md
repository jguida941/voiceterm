...
| `dev/scripts/checks/check_commit_message_row_id_resolves.py` | Commit-message row-id resolver guard | Enforces post-mandate commit-message references to durable plan rows, rejects PKT-BIND-only packet decomposition even when row-prefix checks are not enforced for that row family, catches corrupted row titles, requires applied/completed rows to carry commit-anchor evidence, and reports the scanned range plus active mandate prefixes. |
