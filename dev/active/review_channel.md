...
14. `MP-377` owns continuation-budget and push-readiness authority
    ...
    That includes persisted push/publication truth: if MP-377 already records
    `published_remote`, MP-355 must re-read that state before telling a
    conductor the push is still pending.
15. MP-355 owns the live review projection for typed
    ...
    authority source. Review-channel recovery should consume persisted
    MP-377 push-state truth, not infer unresolved publication from a stale
...
- [x] Keep the transitional `bridge.md` compatibility projection push-safe
      ...
      verification contract. Same-lane recovery now also re-reads the managed
      latest push artifact through `PushEnforcement`, so a completed remote
      publication does not regress back into "push still pending" when the
      local push session or post-push follow-up is interrupted.
...
|---|---|---|---|---|
| `2026-04-17T23:57:48Z` | `CODEX` | Closed the next remote-control receipt-commit continuity gap in prepared conductor authority. `assess_prepared_launch_authority()` now derives post-launch remote-control mode from typed governance/review-state evidence instead of trusting only `DEVCTL_OPERATOR_INTERACTION_MODE`, so commit-driven `HEAD` drift downgrades to `refresh_recommended` only when the live review state actually proves remote-control continuity. Added focused launch-authority regressions for the typed remote-control attachment path and the fail-closed non-remote path; existing stale-authority/reclaimable-session regressions stay green. | `partial-pass` | Run the tooling/docs guard bundle, then re-check the live remote-control review loop to see whether the remaining blocker is still daemon/conductor persistence or the broader typed liveness-state-machine follow-up Claude flagged in `rev_pkt_0995`. |
...
| `2026-04-04T16:03:59Z` | `CODEX` | Closed the next live reviewer-bootstrap teaching gap exposed by the repaired dual-agent loop. Fresh Codex conductor prompts/guards now distinguish ordinary reviewer startup receipts (`action=continue_editing` + `reason=review_pending`, or `action=await_review` + `reason=review_pending_before_push`) from true reviewer-loop repair, so Step 0 still runs but those non-zero reviewer receipts now flow into `review-channel --action status` plus reviewer-owned heartbeat refresh instead of widening into generic repair work. Focused launch/prompt regressions are green on the current tree. | `partial-pass` | Re-run the maintainer docs/tooling guards, then retry a fresh repo-owned Codex launch so the next reviewer session proves it stays in the review loop instead of detouring into repair-only bootstrap. |
| `2026-04-04T08:35:00Z` | `CODEX` | Closed the next live launcher portability gap exposed by the repaired dual-agent session. The generated Claude conductor script no longer assumes subscription-gated `--permission-mode auto` for balanced/strict review-channel launches; it now uses Claude's provider-default permission mode so repo-owned Terminal bootstrap stays live on plans without auto mode while trusted launches still keep the explicit bypass path. Focused launch topology/launch command regression coverage now locks the plan-safe Claude script shape. | `partial-pass` | Re-run the focused launch/recover/status proofs, then retry the live Claude conductor bootstrap to confirm the repo-owned loop advances past `waiting_for_user_input` on the current machine. |
| `2026-04-04T08:03:36Z` | `CODEX` | Closed the remaining sole-authority gap for the recovery slice. `review_state_from_payload()` now normalizes `attention` from `recovery_assessment` whenever both exist and records a migration warning instead of silently accepting divergent legacy input, reviewer-runtime snapshot attachment now overwrites report `attention` from typed `ReviewState` before building doctor output, and `check_review_surface_consistency.py` now carries an explicit attention-projection parity contract so raw `review_state` payloads fail when headline attention drifts from canonical diagnosis/decision fields. Focused parser/event/doctor/parity regressions cover normalization, writer precedence, and guard failure on drift while legacy attention-only artifacts still parse. | `partial-pass` | Re-run the focused recovery-authority suites plus the broad CI bundle, then keep the remaining red set honest as branch/runtime blockers only (`dashboard.py` subprocess policy debt and the live reviewer-loop freshness/ACK state). |
| `2026-04-04T02:35:00Z` | `CODEX` | Landed the first typed recovery-authority routing slice over the live review-channel surfaces. `ReviewState` now carries a typed `recovery_assessment` (`RecoveryDiagnosis` + `RecoveryDecision`), the bridge-backed/event-backed emitters build that object once from typed bridge/runtime truth, legacy `attention` becomes a projection of it, doctor/startup repair/startup summary/turn-authority/bridge-poll now consume the same assessment instead of recomputing local recovery prose, and `check_review_surface_consistency.py` now enforces diagnosis/decision parity plus "no green doctor while diagnosis is degraded." Focused runtime/guard/startup/bridge-poll/platform tests are green and the dedicated review-surface consistency guard passes on the current tree. | `partial-pass` | Keep the next slice bounded to shape/merge readiness: split the new recovery-authority helpers to satisfy code-shape budgets, shrink the widened `turn_authority`/parser files, and decide whether the recovery-assessment field should remain on `ReviewState` or move behind a contract-row/version update before claiming this slice is fully merge-ready. |
...
| `2026-04-03T02:34:35Z` | `CODEX` | Landed the first daemon-liveness closure on the live launch path. `review-channel --action launch|rollover --terminal terminal-app` now auto-starts the repo-owned ensure-follow publisher plus the reviewer-supervisor runtime and fails closed if that detached runtime does not come up. The same bounded change keeps the phone-steered remote-control wrapper on the sanctioned launch path instead of relying on a second manual `ensure --follow` step. Added focused launch/runtime regressions for the new wiring. | `partial-pass` | Run the broader MP-355/tooling guard bundle, then land the launchd crash-restart slice and doctor CLI registration as separate commit-sized follow-ups. |
| `2026-04-02T23:40:00Z` | `CODEX` | Landed the typed `ReviewerRuntimeContract` owner slice the architecture audit called out. `ReviewState` now carries one reviewer-lifecycle contract with explicit mode/effective-mode, freshness/stale reason, last-poll data, rollover state, session ownership, recovery permission, review acceptance, and publish-clear truth. Bridge `review_accepted`, doctor/status payloads, reviewer-follow recovery/rollover logic, and startup push-readiness now all read that contract as the authority instead of recomputing separate bridge-local heuristics. The same slice filled `startup_surface_tokens` across all 14 platform contract rows so bootstrap surfaces can advertise the governed contract map consistently. | `partial-pass` | Run the platform-contract closure and broader repo guard bundle on the updated runtime surfaces, then checkpoint this MP-355 slice once those validations stay green. |
| `2026-04-02T22:35:00Z` | `CODEX` | Closed the operator-reported terminal-cleanup gap in the live rollover path without inventing a second lifecycle surface. Terminal.app launch now returns the launched window id and writes it into `PreparedSessionRecord` session metadata/report payloads, `session_probe` snapshots both `terminal_window_id` and the retiring conductor pid before rollover overwrites the live metadata files, and live `review-channel --action rollover --terminal terminal-app` now reaps retired conductor sessions by killing the old pid before closing the empty Terminal window. Added focused regressions for launch-line output, metadata capture, session-probe loading, cleanup ordering, rollover cleanup dispatch, and launch-topology metadata shape. | `partial-pass` | Run the repo-owned MP-355/tooling guard bundle, then keep the remaining review-channel honesty gaps explicit instead of over-claiming closure from this bounded terminal slice alone. |
| `2026-04-02T17:30:00Z` | `CODEX` | Closed the false-fresh reviewer-follow gap behind the operator-observed 15-minute idle reviewer stall. `ensure --follow` and `reviewer-heartbeat --follow` were only refreshing reviewer heartbeat and never turning `review_needed=true` back into reviewer work, so detached/automation-only runtime state could still look alive while the worktree diverged. The accepted containment stays inside the existing MP-355 state machine: automation-only reviewer heartbeat writes are suppressed while review is pending, and the same follow path now queues a Claude-targeted `restore_reviewer_turn` packet through the existing review-channel event store when pending review collides with detached/automation-only reviewer truth. Focused review-channel regressions are green and the late-bound follow-runtime test seam is now covered correctly. | `partial-pass` | Keep MP-355 honest about the remaining client/operator gap: land the remote-wrapper dry-run side-effect fix first, then expose the remote-orchestration contract map in startup/prompt/bootstrap outputs instead of inventing new bridge-local state. |
...
| `2026-03-28T13:52:00Z` | `CODEX` | Closed the remaining nested-heading instruction leak after the bridge-safe promotion fix. Event-backed queue/current-session projections were still building `derived_next_instruction` with raw `## Context Recovery Packet` markdown, which could reintroduce malformed nested headings in `latest.md`-style outputs even when `bridge.md` stayed clean. Switched the event-backed instruction path to the same compact no-H2 context summary used by promotion and kept the full packet only in source metadata for prompts/audit surfaces. | `partial-pass` | Re-run the focused review-channel projection tests plus governed tooling validation, then refresh reviewer/runtime state and push through the repo-owned path. |
...
| `2026-03-27T08:50:00Z` | `CODEX` | Recorded the next architecture boundary failure immediately after the bounded consumer-refresh checkpoint. The repo briefly tried to answer `MP-355` / `MP-377` startup-review/operator-control churn by teaching it through VoiceTerm end-user docs, which is the wrong repair and the wrong product boundary. Pulled that AI-system/operator prose back out of the VoiceTerm guides, promoted the problem into the `MP-377` owner chain plus `dev/audits/architecture_alignment.md`, and added an explicit boundary rule here so future review-channel sessions route this class of change into plan/maintainer/generated surfaces first. | `in-progress` | Resume from the organization/docs-policy split, not another prose patch: classify product docs vs self-hosting/development docs vs portable adopter docs vs generated/compatibility surfaces, then land the policy/guard changes that stop review-channel/startup control-plane work from forcing VoiceTerm user-doc churn. |
...
- Next action: keep the owner split explicit for the active repair lane.
  ...
  `bridge-poll` turn routing over that runtime/current-session state plus the
  bridge-only reviewer wait markers. Do not move `bridge-poll` back to local
  branch logic or bloat `ReviewerRuntimeContract` with next-turn fields.
...
- Next action: this bounded rollover follow-up is now in. Use the stored
  ...
  ad hoc close-window AppleScript without the pre-kill step.
...
- Current status: this plan remains active; start from the highest-priority
  ...
  `## Progress Log`.
- Boundary rule: if a review-channel/startup/current-session change seems to
  require new prose in VoiceTerm end-user docs (`README`, `QUICK_START`,
  `guides/USAGE`, `guides/CLI_FLAGS`, `guides/INSTALL`,
  `guides/TROUBLESHOOTING`), stop and route that note into the `MP-377`
  owner chain first. Those product docs are not the home for AI-system
  self-hosting or control-plane authority.
- Next action: keep the remaining writer/mutation side of the typed
  ...
  `bridge.md` can stop acting like a live freshness source. The newest same-
  lane closure is now in: active-dual-agent reviewer checkpoints also require
  the live typed `implementer_state_hash`, and bridge-backed status/poll
  surfaces emit that hash so stale reader failures are explicit instead of
  hidden behind manual reread habits. The bounded
  ...
  that already powers the live status surfaces. The shared typed
  `PushDecisionState` splice is now in, so the next same-lane slice must also stop
