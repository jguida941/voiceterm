...
- Next action: treat `ReviewState.reviewer_runtime` as the only reviewer-lifecycle owner for this lane. Any future bridge `review_accepted`, doctor/status fields, reviewer-follow recovery/rollover decisions, or startup push gating should project from that typed contract instead of re-parsing bridge prose or rebuilding parallel reviewer-state heuristics.
