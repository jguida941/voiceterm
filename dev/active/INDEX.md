...
| `dev/active/contract_orphans_audit.md` | `reference` | `reference-only` | follow-up backlog for the 60 orphan contracts flagged by `check_contract_consumer_coverage_sweep`; lists top offender modules and the wire-or-collapse plan items required to drain the 97 sweep violations | when planning typed-contract orphan remediation; consult before scheduling a slice that wires runtime/review_channel/platform contract dataclasses to external readers and writers |
| `dev/active/live_state_semantic_tdd_plan.md` | `reference` | `reference-only` | Live-State Semantic TDD plan (2026-05-23): the durable record of how the typed live-state invariant suite at `dev/scripts/devctl/tests/scenarios/test_live_state_invariants.py` composes with the existing `GroundTruthProbeRunReceipt` and `final_response_gate` proof chain. Captures Phase F autonomous-run progress: topology architecture migration, A19 stale-packet hygiene, A19 delivery_emitted_at_utc lifecycle, A19 stale_packet_count vs warning divergence, A30/A35 orphan-guard fake-green ratchet, A18 reviewer-mutation regression guard, AntiDumbass declared/authority drift, A20 operator-mandate ack TTL, A17 body_open_required ratchet, A25 connectivity sweep meta-invariant, A4 voiceterm leakage ratchet, A12 FPR proof-of-test ratchets, and SYSTEM_MAP orphan-reference ratchet. | when adding new live-state invariants or amending the dogfood/proof workflow; read alongside the standalone wedge demo at github.com/jguida941/semantic-tdd |
| `dev/active/semantic_tdd_lane.md` | `reference` | `reference-only` | Owner doc for the semantic-TDD lane practice (RED → fix → GREEN → ROUTER → DOGFOOD → RECEIPT) and the typed proof matrix it produces across `dev/scripts/devctl/tests/scenarios/`. Migrates the proof matrix from the `MP-GUARDIR-V4-PHASE-0-6-E-CURRENT-PLAN-AUTHORITY-S1` operator staging file (reviewer/packet route guards G23-G29, role cardinality / shared round state G30-G39, STOP gate, FeatureProofReceipt ratchet, plus the Codex / peer-spawn dogfood matrix asserting typed denial shape and bypass-grant scope transitivity). Read alongside `dev/active/live_state_semantic_tdd_plan.md`; that doc is scoped to one test file and this lane covers the broader scenarios-suite practice and peer-spawn dogfood discipline. | when adding a new scenario test under `dev/scripts/devctl/tests/scenarios/`, sweeping a proof-matrix row past `GREEN_LOCAL`, or running the Codex / peer-spawn dogfood loop |
| `dev/active/receipt_steward_lane.md` | `reference` | `reference-only` | Owner doc for the typed `receipt_steward` GOVERNANCE/audit-only role substrate (A38.2 S1). Captures the 6-phase audit ritual (DISCOVER_SLICE → INVENTORY_EVIDENCE_PATHS → VERIFY_RECEIPT_PRESENT → VERIFY_PYTEST_NODE_RESOLVABLE → VERIFY_COMMIT_SHA_LINKED → EMIT_AUDIT_RECEIPT), the `ReceiptStewardAuditReceipt` output shape with its 7-value `missing_items` taxonomy, and the scope split between S1 (substrate only) and S2..S4 (CLI, scope-claim lifecycle, gate composition, exemption lifecycle). | when extending the receipt-steward substrate, wiring CLI surfaces over the role spec, or auditing per-slice FeatureProofReceipt coverage gaps |
| `dev/active/system_map_steward_lane.md` | `reference` | `reference-only` | Owner doc for the typed `system_map_steward` GOVERNANCE/audit-only role substrate (A38.3 S1). Captures the 6-phase platform-coverage audit ritual (LOAD_PLATFORM_INVENTORY → DETERMINE_SLICE_RELEVANCE → AUDIT_CONNECTIONS → SYNTHESIZE_GAPS → PROPOSE_SYSTEM_MAP_UPDATE → EMIT_COVERAGE_AUDIT_RECEIPT), the 15 PLATFORM-COMPONENT audit dimensions (NOT TDD steps — see evidence.md Case 11), the `PlatformCoverageAudit` output shape, and the unification of the legacy `system_alignment_role` placeholder via `_ROLE_ID_ALIASES`. | when extending the system-map-steward substrate, wiring CLI surfaces over the role spec, auditing per-slice platform-coverage gaps, or proposing rows for `dev/guides/SYSTEM_MAP.md` |
