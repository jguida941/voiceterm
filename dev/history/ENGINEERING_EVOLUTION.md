...
- Maintainer-facing workflow docs were rewritten in plain language (`What checks protect us`, `When to push where`) so developers can quickly choose the right local checks and push path without translating policy terms, while keeping `AGENTS.md` as the canonical bundle/router source. Evidence: `dev/DEVELOPMENT.md`, `AGENTS.md`, `dev/scripts/README.md`, `dev/active/MASTER_PLAN.md`.
