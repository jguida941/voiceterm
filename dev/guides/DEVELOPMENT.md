...
- The same Phase-2 authority cleanup now keeps review/push truth on typed
  ...
  `approved_target_identity` tree receipt instead of raw HEAD equality. The
  push identity finding path must consume the same managed receipt-chain proof
  as `publication_authorization_decision`, so preflight-created receipt commits
  do not self-invalidate a still-current authorization.
...
- Compatibility shims now use the same split governance model everywhere:
  ...
  enforce the selected roots globally. Root-scoped release checks also filter
  flat-root and crowded-directory violations from non-selected roots so old
  branch-range debt outside the ratchet target remains visible but non-blocking.
  Repo policy may ratchet those known crowded
...
- Keep one workflow, not a dev-vs-agent fork:
  ...
  - A session that emits TASK_COMPLETE only continues when typed termination
...
- An explicit launch `--reviewer-mode active_dual_agent` must restore typed
  ...
  - `devctl pipeline --action refresh-authorization` is only the same-HEAD
    ...
    contiguous chain of governed bridge/ReviewSnapshot or policy-owned
    generated-surface receipt commits sits
    above the authorized pipeline commit, pipeline status must report
    `head_movement_classification=managed_receipt` with
    `head_has_moved=false`, and push authorization/startup recovery must match
    the active `PushAuthorizationRecord` through that managed chain. Downstream
    push identity findings use the same managed-chain decision flag; do not add
    a second raw-HEAD equality check that contradicts the authorization gate.
...
- The managed pre-commit hook is read-only: it evaluates the typed
  ...
  Post-commit ReviewSnapshot receipt refresh remains fail-open and
  time-bounded. `DEVCTL_REVIEW_SNAPSHOT_TIMEOUT_SECONDS` defaults to
  90 seconds (`0` disables the timeout) so
  `review-snapshot --write --receipt-commit` cannot leave an otherwise-allowed
  commit waiting indefinitely; freshness guards still decide whether the
  receipt chain is acceptable before handoff or publication.
...
| Agent boot-card projection contracts | `python3 dev/scripts/checks/check_agents_contract.py` + `python3 dev/scripts/checks/check_agents_bundle_render.py` | `tooling_control_plane.yml` |
...
| SYSTEM_MAP contract-registry coverage drift | `python3 dev/scripts/checks/check_systemmap_covers_contract_registry.py` (`python3 dev/scripts/devctl.py render-surfaces --write --surface system_map_index --format md` to regenerate) | `tooling_control_plane.yml` + `release_preflight.yml` |
...
- `devctl check-router --execute --keep-going` also reports a typed execution
  ...
  sessions with a 420s measured floor plus measured per-target overrides for
  known-heavy files, so heavy pytest files do not compete inside one combined
  session. Routed commands also carry timeout policy; a timeout is a failed guard result
...
- `devctl progress-status --format md` is the read-only progress surface for
  ...
  runner writes typed `StageProgressEvent` start/heartbeat/completion rows
  under `dev/reports/progress/`, so a quiet guard or preflight can be inspected
  from typed state instead of ad hoc `ps` polling.
...
# Optional: make the routed execution budget explicit when measuring a slow guard
...
- `python3 dev/scripts/checks/check_guard_enforcement_inventory.py` blocks registered check scripts from drifting out of bundle/workflow enforcement lanes unless they are enforced by a governed internal pipeline lane or explicitly marked helper-only, manual-only, contextual-input-only, or temporary advisory backlog exceptions. Shared hard guards should also appear in the resolved `quality-policy` inventory so `devctl check`, generated surfaces, and workflow/bundle enforcement all describe the same lane.
- `python3 dev/scripts/checks/check_bridge_projection_only.py` is a shared hard guard, not a catalog-only advisory. The root entrypoint is a stable compatibility shim over `dev/scripts/checks/bridge_projection_only/command.py` so the crowded checks root stays layout-ratcheted while callers keep the documented command. It runs from the quality-policy defaults, `bundle.tooling`, `bundle.release`, `tooling_control_plane.yml`, and `release_preflight.yml` so repaired bridge/status/current-session projection seams cannot quietly become backend authority again.
- `python3 dev/scripts/checks/check_runtime_bridge_projection_separation.py` is the report-only companion for bridge-reader migration. It scans `dev/scripts/devctl/runtime/`, `dev/scripts/devctl/review_channel/`, and `dev/scripts/devctl/commands/`, reports bridge projection imports/calls as `BridgeSeparationGuard` evidence, and must remain non-enforcing until the P188 typed snapshot, renderer, absent-bridge dogfood, and reader migration baselines are explicit.
...
- `python3 dev/scripts/checks/check_agents_bundle_render.py` is the backward-compatible AGENTS boot-card projection guard; it delegates to `render-surfaces` for `agents_boot_card` instead of treating AGENTS as command-bundle authority.
- `python3 dev/scripts/checks/check_instruction_surface_sync.py` enforces generated boot-card instruction content in addition to drift: role choices must stay explicit, plan/policy/artifact/runtime packet writes must mention typed `--target-kind` / `--target-ref` fields, and long reducer commands should point agents to `Operator Command Wrappers` instead of brittle one-line shell reconstruction.
...
- `python3 dev/scripts/checks/check_review_snapshot_freshness.py` blocks stale `dev/audits/REVIEW_SNAPSHOT.md` by comparing the HEAD SHA and generation stamp embedded in the file against the live typed projection. The guard also accepts a governed receipt chain when the generated snapshot binds to that receipt commit's parent code state or any ancestor in a contiguous managed bridge/ReviewSnapshot/policy-generated-surface receipt chain, because a file inside a commit cannot contain its own final SHA. If non-receipt HEAD or stamp drift occurs, the guard fails and instructs the caller to rerun `devctl review-snapshot --write --receipt-commit`; both `tooling_control_plane.yml` and `release_preflight.yml` run it. The managed raw-git path is three-hook automation: `install-git-hooks` installs a read-only pre-commit commit-permission hook, a post-commit receipt hook that calls `devctl review-snapshot --write --receipt-commit`, and a blocking pre-push hook that refuses raw `git push` unless the nested push came from `devctl push --execute`. The pre-commit hook fails closed when the typed `commit_permission` boundary blocks raw `git commit`; it never refreshes, stages, or writes ReviewSnapshot/bridge projections while git is preparing the commit index. Governed staging must preserve any already-staged user paths while adding the refreshed snapshot artifact, and `devctl commit` may surface the main content SHA separately from the trailing receipt SHA when the post-commit receipt lands. `devctl push` accepts that receipt shape when the receipt HEAD, or any contiguous managed bridge/ReviewSnapshot/generated-surface receipt ancestor above it, leads back to the active `PushAuthorizationRecord`; it now runs `render-surfaces --write` before routed preflight, refreshes ReviewSnapshot, commits tracked non-local repo-pack surface drift and managed ReviewSnapshot/`bridge.md` projection drift as pathscoped receipts before the preflight gate, runs `review-snapshot --write --receipt-commit` when that batch moves `HEAD` unless `HEAD` is already a managed ReviewSnapshot/generated-surface receipt recognized through the shared receipt-prefix registry, refreshes the event-backed review-channel projection bundle plus startup/context-graph surfaces after managed receipt movement, and then refreshes the current HEAD before authorization/reporting so green pushes do not hand off tracked projection drift. Managed-projection dirty detection must parse `git status --porcelain` rows even after the shared git helper trims a leading status space; an unstaged `REVIEW_SNAPSHOT.md` row may arrive as `M dev/audits/REVIEW_SNAPSHOT.md`, and governed push must still treat it as receipt-owned dirt instead of reporting `post_push_green` over a dirty worktree. The later approved-target identity finding path uses the same managed-chain authorization decision instead of re-blocking on raw content-commit-vs-receipt-HEAD inequality. Startup push-state uses the same managed receipt artifact set when computing dirty-source counts, and `push_enforcement` separates unpublished source commits from managed receipt commits so receipt-chain buildup is publication backlog rather than post-checkpoint source dirt. `python3 dev/scripts/checks/check_system_picture_freshness.py` is the companion pre-push freshness gate: it fails stale `SystemPicture` sections and requires the startup and graph sections to be current, which means `startup-context` and `context-graph --mode bootstrap` must be refreshed after each commit before publication. `devctl push` ignores staged-only "next commit" intent when evaluating push dirt or preflight auto-commit repair, and selected generated-surface receipt commits use pathspecs so staged-only work stays out of the machine receipt. Generated-surface receipts may set `DEVCTL_MANAGED_PROJECTION_RECEIPT_COMMIT=1` for downstream receipt-chain classification, but source commits and stale or mismatched handoff evidence still fail closed through `commit_permission`. Stale detached pipeline records are ignored in `single_agent` mode; active dual-agent and current pipeline targets still require exact typed authorization. The typed snapshot now also carries first-class probe run-state/artifact refs plus current push receipt/authorization refs so external review surfaces can cite emitted evidence instead of only replaying next-command suggestions.
- Publication authorization is read from the canonical event-backed
  `dev/reports/review_channel/projections/latest/commit_pipeline.json` first.
