...
- The managed ReviewSnapshot hook refreshes are fail-open and time-bounded.
  ...
  post-commit `review-snapshot --write --receipt-commit` cannot leave an
  otherwise-allowed commit waiting indefinitely; freshness guards still decide
  whether the receipt chain is acceptable before handoff or publication.
