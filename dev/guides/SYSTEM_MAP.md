...
|---|---:|---|
| `dev/scripts/devctl` | 2362 | commands=608, tests=574, runtime=426, review_channel=308, platform=83, governance=74, (root)=62, context_graph=47 |
...
- source_contract_count: 168
- connected_contract_count: 168
- rendered_contract_count: 21
- omitted_contract_count: 147
...
- `PacketAttentionIngestionDecision`, `PacketBacklogPressure`, `PacketDebtRemediationReport`, `PacketDurableIngestionReceipt`, `PacketGuardErrorDetail`, `PacketIngestDecision`, `PacketIntentAnchor`, `PacketIntentClassification`
...
- `RelationshipGraph`, `RelaunchQuotaExceeded`, `RemoteCommitPipelineContract`, `RemoteControlAttachmentState`, `RemoteControlCollaborationCampaign`, `RemoteControlInvocationReceipt`, `RepoPack`, `RepoPortabilityCheck`
- `ResolutionReceipt`, `ReviewCandidateRecord`, `ReviewChannelCommandFreshness`, `ReviewPacket`, `ReviewState`, `ReviewTargets`, `ReviewerAuditReceipt`, `ReviewerResponseShape`
- `ReviewerRuntimeContract`, `RoleCapability`, `RoleCapabilityRegistry`, `RoleCommandEnvelope`, `RoleCreationAction`, `RoleGuard`, `RoleInstructionCard`, `RunRecord`
- `RuntimeErrorReceipt`, `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`, `SecurityReport`, `SessionActivityEntry`, `SessionActivityLog`, `SessionCachePacket`
...
- `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StalledQueueFinding`, `SystemPicture`, `TaskCompleteDecision`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`
- `TokenOptimizationDogfoodReceipt`, `TransitionContract`, `TypedAction`, `TypedGateFailure`, `TypedNamespaceCompositionGuard`, `TypedOutputHumanSummary`, `WorkPublicationLedger`, `WorkPublicationLedgerEvent`
