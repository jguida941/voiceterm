...
|---|---:|---|
| `dev/scripts/devctl` | 2493 | commands=624, tests=624, runtime=477, review_channel=318, platform=86, governance=74, (root)=63, context_graph=47 |
| `dev/scripts/checks` | 555 | (root)=181, package_layout=32, platform_contract_closure=22, review_probes=21, multi_agent_sync=19, python_analysis=17, code_shape=15, rust_analysis=15 |
...
- source_contract_count: 210
- connected_contract_count: 210
- rendered_contract_count: 21
- omitted_contract_count: 189
...
- `OrphanSource`, `OrphanSourceClassification`, `OrphanSourceDecision`, `PacketADRReceipt`, `PacketAttentionIngestionDecision`, `PacketBacklogPressure`, `PacketBatchTriage`, `PacketBatchTriageRow`
- `PacketDebtRemediationReport`, `PacketDebtRemediationRow`, `PacketDurableIngestionReceipt`, `PacketGuardErrorDetail`, `PacketIngestDecision`, `PacketIntentAnchor`, `PacketIntentClassification`, `PacketKindTtl`
- `PacketPlanIngestionMapping`, `PacketReadReceipt`, `PacketSupersessionLink`, `PacketUrgencyClassification`, `PeerAwarenessDecision`, `PeerAwarenessPolicy`, `PeerHeartbeatEvidence`, `PlanIntentIngestionReceipt`
...
- `ReviewState`, `ReviewTargets`, `ReviewerAuditReceipt`, `ReviewerResponseShape`, `ReviewerRuntimeContract`, `RoleCapability`, `RoleCapabilityRegistry`, `RoleCommandEnvelope`
- `RoleCreationAction`, `RoleGuard`, `RoleInstructionCard`, `RoleReviewAssignmentLifecycle`, `RoleReviewReceipt`, `RoleReviewTimeout`, `RunRecord`, `RuntimeErrorReceipt`
- `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`, `SchemaMigrationSpine`, `SecurityReport`, `SessionActivityEntry`, `SessionActivityLog`, `SessionCachePacket`
- `SessionLease`, `SessionLivenessReconciler`, `SessionLivenessSignal`, `SessionOrientationPacket`, `SessionPosture`, `SessionStatusProjection`, `SessionTerminationPolicy`, `SkillCompatibilityValidator`
- `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StaleFindingReceipt`, `StaleProofRef`, `StalledQueueFinding`, `SystemMapSnapshot`, `SystemPicture`
- `TagReceipt`, `TaskCompleteDecision`, `TaskProducedTtlEvidence`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`, `TokenOptimizationDogfoodReceipt`, `TransitionContract`, `TypedAction`
- `TypedGateFailure`, `TypedNamespaceCompositionGuard`, `TypedOutputHumanSummary`, `TypingCompatUtility`, `WorkPublicationLedger`, `WorkPublicationLedgerEvent`, `WorkPublicationLedgerHeader`, `WorkflowAdapter`
- `WorkflowStep`, `WorktreeBaseline`
