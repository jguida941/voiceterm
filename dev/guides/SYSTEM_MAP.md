...
|---|---:|---|
| `dev/scripts/devctl` | 2372 | commands=611, tests=576, runtime=428, review_channel=309, platform=84, governance=74, (root)=63, context_graph=47 |
| `dev/scripts/checks` | 497 | (root)=159, package_layout=32, platform_contract_closure=22, review_probes=21, multi_agent_sync=18, python_analysis=17, code_shape=15, rust_analysis=15 |
...
- source_contract_count: 178
- connected_contract_count: 178
- rendered_contract_count: 21
- omitted_contract_count: 157
...
- `LocalServiceEndpoint`, `MandatoryIngestBeforeImplementInvariant`, `ManualBypassImportReceipt`, `ModeChangeReceipt`, `OperatorCommandWrapper`, `OrphanInventoryReport`, `OrphanReconciliationDecision`, `OrphanSnapshot`
...
- `PlanIterationSession`, `PlanRowContractRefsResolveGuard`, `PlanSourceSnapshot`, `PlatformContractRegistry`, `PlatformContractRegistryRow`, `PlatformFindingIngest`, `PlatformGuide`, `PreDecisionComposabilityWindow`
- `ProbeAllowlist`, `ProbeReport`, `ProviderAdapter`, `PublicationEpisode`, `PushAuthorizationRecord`, `QualityFeedbackSnapshot`, `QueueBypassReasoning`, `RawGitBypassReceipt`
- `RelationshipGraph`, `RelaunchQuotaExceeded`, `RemoteCommitPipelineContract`, `RemoteControlAttachmentState`, `RemoteControlCollaborationCampaign`, `RemoteControlInvocationReceipt`, `RepairPacket`, `RepoPack`
- `RepoPortabilityCheck`, `ResolutionReceipt`, `ReviewCandidateRecord`, `ReviewChannelCommandFreshness`, `ReviewPacket`, `ReviewState`, `ReviewTargets`, `ReviewerAuditReceipt`
- `ReviewerResponseShape`, `ReviewerRuntimeContract`, `RoleCapability`, `RoleCapabilityRegistry`, `RoleCommandEnvelope`, `RoleCreationAction`, `RoleGuard`, `RoleInstructionCard`
- `RunRecord`, `RuntimeErrorReceipt`, `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`, `SecurityReport`, `SessionActivityEntry`, `SessionActivityLog`
...
- `SkillCompatibilityValidator`, `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StaleFindingReceipt`, `StalledQueueFinding`, `SystemPicture`, `TaskCompleteDecision`
