...
|---|---:|---|
| `dev/scripts/devctl` | 2379 | commands=612, tests=580, runtime=429, review_channel=310, platform=84, governance=74, (root)=63, context_graph=47 |
| `dev/scripts/checks` | 500 | (root)=161, package_layout=32, platform_contract_closure=22, review_probes=21, multi_agent_sync=18, python_analysis=17, code_shape=15, rust_analysis=15 |
...
- source_contract_count: 180
- connected_contract_count: 180
- rendered_contract_count: 21
- omitted_contract_count: 159
...
- `CheckpointBudgetShape`, `CheckpointRepairAuthority`, `ClassifierSafetyAttestation`, `ClosureProof`, `CloudFinding`, `CollaborationModeTopology`, `CommitBodyPacketAnchorsGuard`, `CommitReceipt`
- `ComposesWith`, `ContextGraphDelta`, `ContextGraphSnapshot`, `ContextGraphSnapshotFreshnessGuard`, `ContinuousImprovementMode`, `ContractRef`, `ControlPlaneReadModel`, `ControlState`
- `CoordinationSnapshot`, `CorrelationContext`, `CustomRoleDefinition`, `DecidedPacketDebtDetector`, `DecisionPacket`, `DevelopRoleAdapterSpec`, `DevelopmentCampaignRoleState`, `DevelopmentLoopReport`
- `DevelopmentModeTopology`, `DogfoodSelfCheckReceipt`, `EvidenceArchiveManifest`, `EvidenceArchivePolicy`, `EvidenceArchiveReceipt`, `ExceptionClass`, `ExceptionLifecycleStatus`, `ExceptionPolicy`
...
- `FindingBacklog`, `FindingReview`, `GoalProgressReceipt`, `GovernanceCompatibilityClaim`, `GovernedExceptionLifecycle`, `GovernedTransitionModule`, `GroundTruthProbeRunReceipt`, `IngestionProvenance`
- `LifecycleReceipt`, `LocalServiceEndpoint`, `MandatoryIngestBeforeImplementInvariant`, `ManualBypassImportReceipt`, `ModeChangeReceipt`, `OperatorCommandWrapper`, `OperatorDirectivePacket`, `OrphanInventoryReport`
...
- `PacketIntentAnchor`, `PacketIntentClassification`, `PacketPlanIngestionMapping`, `PacketReadReceipt`, `PacketSupersessionLink`, `PacketUrgencyClassification`, `PeerAwarenessDecision`, `PeerAwarenessPolicy`
- `PeerHeartbeatEvidence`, `PlanIntentIngestionReceipt`, `PlanIterationSession`, `PlanRowContractRefsResolveGuard`, `PlanSourceSnapshot`, `PlatformContractRegistry`, `PlatformContractRegistryRow`, `PlatformFindingIngest`
...
- `RoleGuard`, `RoleInstructionCard`, `RunRecord`, `RuntimeErrorReceipt`, `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`, `SecurityReport`
...
- `SessionStatusProjection`, `SessionTerminationPolicy`, `SkillCompatibilityValidator`, `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StaleFindingReceipt`, `StalledQueueFinding`
- `SystemPicture`, `TaskCompleteDecision`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`, `TokenOptimizationDogfoodReceipt`, `TransitionContract`, `TypedAction`, `TypedGateFailure`
- `TypedNamespaceCompositionGuard`, `TypedOutputHumanSummary`, `WorkPublicationLedger`, `WorkPublicationLedgerEvent`, `WorkPublicationLedgerHeader`, `WorkflowAdapter`, `WorkflowStep`, `WorktreeBaseline`
