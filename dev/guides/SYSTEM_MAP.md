...
|---|---:|---|
| `dev/scripts/devctl` | 2359 | commands=608, tests=571, runtime=426, review_channel=308, platform=83, governance=74, (root)=62, context_graph=47 |
| `dev/scripts/checks` | 494 | (root)=156, package_layout=32, platform_contract_closure=22, review_probes=21, multi_agent_sync=18, python_analysis=17, code_shape=15, rust_analysis=15 |
...
- source_contract_count: 165
- connected_contract_count: 165
- rendered_contract_count: 21
- omitted_contract_count: 144
...
- `CheckpointBudgetShape`, `CheckpointRepairAuthority`, `ClassifierSafetyAttestation`, `ClosureProof`, `CollaborationModeTopology`, `CommitReceipt`, `ComposesWith`, `ContextGraphDelta`
- `ContextGraphSnapshot`, `ContextGraphSnapshotFreshnessGuard`, `ContinuousImprovementMode`, `ContractRef`, `ControlPlaneReadModel`, `ControlState`, `CoordinationSnapshot`, `CorrelationContext`
- `CustomRoleDefinition`, `DecisionPacket`, `DevelopRoleAdapterSpec`, `DevelopmentCampaignRoleState`, `DevelopmentLoopReport`, `DevelopmentModeTopology`, `DogfoodSelfCheckReceipt`, `EvidenceArchiveManifest`
...
- `FileTopology`, `Finding`, `FindingBacklog`, `FindingReview`, `GoalProgressReceipt`, `GovernanceCompatibilityClaim`, `GovernedExceptionLifecycle`, `GovernedTransitionModule`
- `GroundTruthProbeRunReceipt`, `IngestionProvenance`, `LifecycleReceipt`, `LocalServiceEndpoint`, `MandatoryIngestBeforeImplementInvariant`, `ManualBypassImportReceipt`, `ModeChangeReceipt`, `OrphanInventoryReport`
...
- `PacketBacklogPressure`, `PacketDebtRemediationReport`, `PacketDurableIngestionReceipt`, `PacketGuardErrorDetail`, `PacketIngestDecision`, `PacketIntentAnchor`, `PacketIntentClassification`, `PacketPlanIngestionMapping`
...
- `PlanRowContractRefsResolveGuard`, `PlanSourceSnapshot`, `PlatformContractRegistry`, `PlatformContractRegistryRow`, `PlatformFindingIngest`, `PlatformGuide`, `PreDecisionComposabilityWindow`, `ProbeAllowlist`
- `ProbeReport`, `ProviderAdapter`, `PublicationEpisode`, `PushAuthorizationRecord`, `QualityFeedbackSnapshot`, `QueueBypassReasoning`, `RawGitBypassReceipt`, `RelationshipGraph`
- `RelaunchQuotaExceeded`, `RemoteCommitPipelineContract`, `RemoteControlAttachmentState`, `RemoteControlCollaborationCampaign`, `RemoteControlInvocationReceipt`, `RepoPack`, `RepoPortabilityCheck`, `ResolutionReceipt`
...
- `SliceClosureEvent`, `StalledQueueFinding`, `SystemPicture`, `TaskCompleteDecision`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`, `TokenOptimizationDogfoodReceipt`, `TransitionContract`
- `TypedAction`, `TypedGateFailure`, `TypedOutputHumanSummary`, `WorkPublicationLedger`, `WorkPublicationLedgerEvent`, `WorkPublicationLedgerHeader`, `WorkflowAdapter`, `WorkflowStep`
- `WorktreeBaseline`
