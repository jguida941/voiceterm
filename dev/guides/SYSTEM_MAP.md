...
|---|---:|---|
| `dev/scripts/devctl` | 2383 | commands=612, tests=584, runtime=429, review_channel=310, platform=84, governance=74, (root)=63, context_graph=47 |
...
- source_contract_count: 181
- connected_contract_count: 181
- rendered_contract_count: 21
- omitted_contract_count: 160
...
- `PeerAwarenessPolicy`, `PeerHeartbeatEvidence`, `PlanIntentIngestionReceipt`, `PlanIterationSession`, `PlanRowContractRefsResolveGuard`, `PlanSourceSnapshot`, `PlatformContractRegistry`, `PlatformContractRegistryRow`
...
- `QualityFeedbackSnapshot`, `QueueBypassReasoning`, `RawGitBypassReceipt`, `RelationshipGraph`, `RelaunchQuotaExceeded`, `RemoteCommitPipelineContract`, `RemoteControlAttachmentState`, `RemoteControlCollaborationCampaign`
...
- `RoleCreationAction`, `RoleGuard`, `RoleInstructionCard`, `RunRecord`, `RuntimeErrorReceipt`, `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`
- `SecurityReport`, `SessionActivityEntry`, `SessionActivityLog`, `SessionCachePacket`, `SessionLease`, `SessionLivenessReconciler`, `SessionLivenessSignal`, `SessionOrientationPacket`
- `SessionPosture`, `SessionStatusProjection`, `SessionTerminationPolicy`, `SkillCompatibilityValidator`, `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StaleFindingReceipt`
- `StalledQueueFinding`, `SystemPicture`, `TaskCompleteDecision`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`, `TokenOptimizationDogfoodReceipt`, `TransitionContract`, `TypedAction`
- `TypedGateFailure`, `TypedNamespaceCompositionGuard`, `TypedOutputHumanSummary`, `WorkPublicationLedger`, `WorkPublicationLedgerEvent`, `WorkPublicationLedgerHeader`, `WorkflowAdapter`, `WorkflowStep`
- `WorktreeBaseline`
