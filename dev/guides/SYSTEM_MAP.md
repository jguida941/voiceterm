...
### Runtime Spine Closure Matrix

This table is intentionally machine-checked by `check_runtime_spine_closure.py`. A ❌/⚠️ object in the spine is not allowed to be a vague doc reminder: it must name the active owner, contract, producer, consumer, proof lane, graph/context visibility, carry-forward path, and priority that keep the gap visible across packet expiry, session compaction, and multi-agent handoff.
...
|---|---|---|---|---|---|---|---|---|
| RepoPack | `MP377-P0-T22AN-F` plus platform packaging rows | `dev/scripts/devctl/platform/contract_definitions.py` RepoPack contract | `devctl governance-bootstrap` producer | `dev/scripts/devctl/runtime/project_governance.py` consumer | `check_platform_contract_sync.py` plus `check_runtime_spine_closure.py` | `context-graph` and `system-map` | `ContextPack` plus `startup-context` carry-forward | P0 |
| PlanExpectationPacket | `MP377-P0-T22AN-F` and `MP377-P0-T22AM-F` | `dev/scripts/devctl/runtime/plan_expectation_packet.py` planned typed contract | `devctl plan compile` producer from PlanTargetRef and WorkIntakePacket | `dev/scripts/devctl/runtime/agent_loop_decision.py` consumer | `test_plan_expectation_packet.py` plus `check_runtime_spine_closure.py` | `context-graph`, `startup-context`, and `AgentLoopDecision` | `DevSessionPack` and `ContextPack` carry-forward | P0 |
| TypedAction | `MP377-P0-T22AN-F` plus action-result rows | `dev/scripts/devctl/runtime/typed_action.py` typed contract | `devctl check` and governed command producers | `dev/scripts/devctl/runtime/action_result.py` consumer | `check_platform_contract_closure.py` plus `check_runtime_spine_closure.py` | `system-map`, `agent-loop`, and `startup-context` | `PacketContinuityState` and `RunRecord` carry-forward | P0 |
| ActionResult | `MP377-P0-T22AN-F` plus command-result rows | `dev/scripts/devctl/runtime/action_result.py` typed contract | `devctl check` and command executor producers | `dev/scripts/devctl/runtime/run_record.py` consumer | `probe_command_result_contract.py` plus `check_runtime_spine_closure.py` | `context-graph` and `startup-context` | `RunRecord` plus `ContextPack` carry-forward | P0 |
| RunRecord | `MP377-P0-T22AN-F` plus runtime evidence rows | `dev/scripts/devctl/runtime/run_record.py` typed contract | `dev/scripts/devctl/runtime/action_result.py` reducer producer | `dev/scripts/devctl/governance/finding_backlog.py` consumer | `check_platform_contract_closure.py` plus `check_runtime_spine_closure.py` | `context-graph`, `system-map`, and `DevSessionPack` | `DevSessionPack` and `ContextPack` carry-forward | P0 |
| CandidateInvariant | `MP377-P0-T22AN-F` and guard-promotion rows | `dev/scripts/devctl/runtime/candidate_invariant.py` planned typed contract | `governance-review` and probe-report producer | `dev/scripts/checks/check_guard_enforcement_inventory.py` consumer | `test_candidate_invariant.py` plus `check_runtime_spine_closure.py` | `context-graph` and `system-map` | `PacketContinuityState`, `ContextPack`, and plan row carry-forward | P0 |
| SessionDecisionLog | `MP377-P0-T22AN-F` and DevSessionPack rows | `dev/scripts/devctl/runtime/session_decision_log.py` planned typed contract | `review-channel` event reducer producer | `dev/scripts/devctl/runtime/startup_context.py` consumer | `test_session_decision_log.py` plus `check_runtime_spine_closure.py` | `startup-context`, `session-resume`, and `context-graph` | `DevSessionPack` and `ContextPack` carry-forward | P0 |
| ContextPack | `MP377-P0-T22AN-F` plus context-pack rows | `rust/src/bin/voiceterm/memory/types.rs` and `dev/scripts/devctl/runtime/context_pack.py` typed contracts | `devctl startup-context` and `session-resume` producers | `dev/scripts/devctl/runtime/agent_loop_decision.py` and `context-graph` consumers | `test_context_pack.py` plus `check_runtime_spine_closure.py` | `context-graph`, `startup-context`, and `session-resume` | `ContextPack` token-budgeted carry-forward | P0 |
...
|---|---:|---|
| `dev/scripts/devctl` | 2594 | tests=685, commands=635, runtime=496, review_channel=323, platform=89, governance=75, (root)=63, context_graph=47 |
| `dev/scripts/checks` | 650 | (root)=222, package_layout=32, platform_contract_closure=22, review_probes=21, multi_agent_sync=19, python_analysis=17, code_shape=15, rust_analysis=15 |

### Governed Surfaces

| Surface | Renderer | Output | Scope |
|---|---|---|---|
| `agents_boot_card` | `instruction_boot_card` | `AGENTS.md` | tracked |
| `system_map_index` | `system_map_renderer` | `dev/guides/SYSTEM_MAP.md` | tracked |
| `claude_boot_card` | `instruction_boot_card` | `CLAUDE.md` | local-only |
...
| `develop_role_slash_adapters` | `template_file` | `dev/templates/slash/develop/roles.md` | tracked |
| `remote_control_slash_adapters` | `template_file` | `dev/templates/slash/remote-control/commands.md` | tracked |
| `claude_remote_control_hook_settings` | `template_file` | `.claude/settings.json` | tracked |
| `claude_typed_remote_control_command` | `template_file` | `.claude/commands/typed-remote-control.md` | tracked |
...
- source_contract_count: 230
- connected_contract_count: 230
- rendered_contract_count: 21
- omitted_contract_count: 209
- rendered_contract_policy: first 12 contracts plus required authority contracts

| Contract | Owner | Fields | Writer | Consumers |
|---|---|---:|---|---|
| `RepoPack` | `repo_packs` | 3 | `contract:RepoPack:declared` | none |
| `TypedAction` | `governance_runtime` | 8 | `dev.scripts.devctl.runtime.action_contracts:TypedAction` | cli |
| `RunRecord` | `governance_runtime` | 10 | `dev.scripts.devctl.runtime.action_contracts:RunRecord` | none |
| `CorrelationContext` | `governance_runtime` | 3 | `dev.scripts.devctl.runtime.correlation_spine:CorrelationContext` | none |
| `EvidenceArchivePolicy` | `governance_runtime` | 9 | `dev.scripts.devctl.runtime.evidence_archive:EvidenceArchivePolicy` | none |
| `EvidenceArchiveManifest` | `governance_runtime` | 7 | `dev.scripts.devctl.runtime.evidence_archive:EvidenceArchiveManifest` | none |
| `EvidenceArchiveReceipt` | `governance_runtime` | 12 | `dev.scripts.devctl.runtime.evidence_archive:EvidenceArchiveReceipt` | none |
| `EvidenceArchiveRefResolution` | `governance_runtime` | 6 | `dev.scripts.devctl.runtime.evidence_archive_ref_guard:EvidenceArchiveRefResolution` | none |
| `SessionActivityEntry` | `governance_runtime` | 16 | `dev.scripts.devctl.runtime.session_activity_log:SessionActivityEntry` | none |
| `SessionActivityLog` | `governance_runtime` | 11 | `dev.scripts.devctl.runtime.session_activity_log:SessionActivityLog` | none |
| `ReviewerResponseShape` | `governance_runtime` | 19 | `dev.scripts.devctl.runtime.reviewer_response_shape:ReviewerResponseShape` | none |
| `RoleInstructionCard` | `governance_runtime` | 7 | `dev.scripts.devctl.runtime.role_customization:RoleInstructionCard` | none |
| `BypassRequest` | `governance_runtime` | 10 | `dev.scripts.devctl.runtime.lifetime_bypass_mode:BypassRequest` | none |
| `BypassEvaluation` | `governance_runtime` | 10 | `dev.scripts.devctl.runtime.lifetime_bypass_mode:BypassEvaluation` | none |
| `BypassReceipt` | `governance_runtime` | 11 | `dev.scripts.devctl.runtime.lifetime_bypass_mode:BypassReceipt` | none |
| `ClassifierSafetyAttestation` | `governance_runtime` | 12 | `dev.scripts.devctl.runtime.classifier_safety_attestation:ClassifierSafetyAttestation` | none |
| `BypassLifecycle` | `governance_runtime` | 8 | `dev.scripts.devctl.runtime.lifetime_bypass_mode:BypassLifecycle` | none |
| `SessionStatusProjection` | `governance_runtime` | 8 | `dev.scripts.devctl.runtime.session_status_projection:SessionStatusProjection` | none |
| `CheckpointRepairAuthority` | `governance_runtime` | 11 | `dev.scripts.devctl.runtime.checkpoint_repair_authority:CheckpointRepairAuthority` | none |
| `GovernedTransitionModule` | `governance_runtime` | 2 | `dev.scripts.devctl.runtime.governed_transitions:GovernedTransitionModule` | none |
| `TransitionContract` | `governance_runtime` | 8 | `dev.scripts.devctl.runtime.governed_transitions:TransitionContract` | none |

### Platform Contract Registry Coverage

- source_registry: `dev/state/contract_registry.jsonl`
- registry_contract_count: 246
- coverage_policy: every registry `contract_id` is rendered below as a backticked token so `check_systemmap_covers_contract_registry.py` can fail stale architecture docs.
- `AcceptAllOrphansAction`, `AcceptAllOrphansReceipt`, `ActionResult`, `ActionResultStatusDomainGuard`, `ActorAuthorityState`, `AffectedTestSelection`, `AgentDispatchRouter`, `AgentLoopBilateralProtocol`
- `AgentMindConsumptionReceipt`, `AgentRelaunchTrigger`, `AgentResumeReceipt`, `AgentSessionContinuation`, `AgentSessionOutcome`, `AgentSpawnReceipt`, `AgentTerminationReceipt`, `AgentWorkflowSpec`
- `AnchorViolation`, `ArtifactStore`, `AssistantGuideMode`, `AutoModeState`, `AutoRepairReceipt`, `BaselineAuthorityInventoryReceipt`, `BranchOperationReceipt`, `BridgeSeparationGuard`
- `BypassEvaluation`, `BypassExpiry`, `BypassLifecycle`, `BypassReceipt`, `BypassRequest`, `CallerAuthorityPolicy`, `CapabilityGrantState`, `CharterDelivery`
- `CheckNonTrivialOutputProofCommand`, `CheckResult`, `CheckoutInventory`, `CheckoutInventoryClassification`, `CheckoutInventoryRow`, `CheckpointBudgetShape`, `CheckpointRepairAuthority`, `ChildActorScopeViolation`
- `ClassifierSafetyAttestation`, `ClosureProof`, `CloudFinding`, `CollaborationModeTopology`, `CommandOutputReceipt`, `CommitBodyPacketAnchorsGuard`, `CommitMessageRowIdResolvesGuard`, `CommitReceipt`
- `ComposesWith`, `ConnectivityRegistrySnapshot`, `ContextGraphDelta`, `ContextGraphSnapshot`, `ContextGraphSnapshotFreshnessGuard`, `ContinuousImprovementMode`, `ContractRef`, `ContractRegistryCompositeKeyUniqueness`
- `ContractRegistryCompositeKeyUniquenessCommand`, `ControlPlaneReadModel`, `ControlState`, `CoordinationSnapshot`, `CoordinationTopologySnapshot`, `CorrelationContext`, `CustomRoleDefinition`, `DecidedPacketDebtDetector`
- `DecisionPacket`, `DecisionTtlEvidence`, `DelegationViolation`, `DevelopRoleAdapterSpec`, `DevelopmentCampaignRoleState`, `DevelopmentLoopReport`, `DevelopmentModeTopology`, `DogfoodSelfCheckReceipt`
- `DurableSchemaPolicy`, `EvidenceArchiveManifest`, `EvidenceArchivePolicy`, `EvidenceArchiveReceipt`, `EvidenceArchiveRefResolution`, `ExceptionClass`, `ExceptionLifecycleStatus`, `ExceptionPolicy`
- `ExceptionReceipt`, `ExpiryRefreshViolation`, `ExtensionBundle`, `FailurePacket`, `FeatureLifecycleProof`, `FeatureProofReceipt`, `FileTopology`, `Finding`
- `FindingAffectedScope`, `FindingApplicability`, `FindingBacklog`, `FindingReview`, `FindingTtlEvidence`, `GitMutationProofReceipt`, `GoalProgressReceipt`, `GovernanceCompatibilityClaim`
- `GovernedExceptionLifecycle`, `GovernedTransitionCheck`, `GovernedTransitionError`, `GovernedTransitionInput`, `GovernedTransitionModule`, `GroundTruthProbeRunReceipt`, `HygieneViolation`, `IllegalTransition`
- `IngestionProvenance`, `LeaseConflictViolation`, `LifecycleReceipt`, `LocalServiceEndpoint`, `LooseChatViolation`, `MandatoryIngestBeforeImplementInvariant`, `ManualBypassImportReceipt`, `MergeConflictViolation`
- `MergeGateViolation`, `MissingDecisionRefreshHint`, `ModeChangeReceipt`, `NonTrivialOutputProof`, `NonTrivialOutputProofRemediationFinding`, `NonTrivialOutputProofRemediationFindingLedger`, `OperatorCommandWrapper`, `OperatorDirectivePacket`
- `OrphanInventoryReport`, `OrphanReconciliationDecision`, `OrphanSnapshot`, `OrphanSnapshotStats`, `OrphanSource`, `OrphanSourceClassification`, `OrphanSourceDecision`, `PacketADRReceipt`
- `PacketAttentionDrainReport`, `PacketAttentionIngestionDecision`, `PacketBacklogPressure`, `PacketBatchTriage`, `PacketBatchTriageRow`, `PacketBodyRouteViolation`, `PacketDebtRemediationReport`, `PacketDebtRemediationRow`
- `PacketDurableIngestionReceipt`, `PacketGuardErrorDetail`, `PacketIngestDecision`, `PacketIntentAnchor`, `PacketIntentClassification`, `PacketKindTtl`, `PacketObservationReceipt`, `PacketPlanIngestionMapping`
- `PacketReadReceipt`, `PacketSupersessionLink`, `PacketUrgencyClassification`, `PeerAwarenessDecision`, `PeerAwarenessPolicy`, `PeerHeartbeatEvidence`, `PeerLeaseVisibilityViolation`, `PlanIntentIngestionReceipt`
- `PlanIterationSession`, `PlanRowClosureReceipt`, `PlanRowContractRefsResolveGuard`, `PlanSourceSnapshot`, `PlanningIRSnapshot`, `PlatformContractRegistry`, `PlatformContractRegistryRow`, `PlatformFindingIngest`
- `PlatformGuide`, `PreDecisionComposabilityWindow`, `ProbeAllowlist`, `ProbeReport`, `ProviderAdapter`, `PublicationEpisode`, `PushAuthorizationRecord`, `QualityFeedbackSnapshot`
- `QuestionTtlEvidence`, `QueueBypassReasoning`, `RawGitBypassReceipt`, `RelationshipGraph`, `RelaunchQuotaExceeded`, `RemoteCommitPipelineContract`, `RemoteControlAttachmentState`, `RemoteControlCollaborationCampaign`
- `RemoteControlInvocationReceipt`, `RepairPacket`, `RepoPack`, `RepoPortabilityCheck`, `ResolutionReceipt`, `ReviewCandidateRecord`, `ReviewChannelCommandFreshness`, `ReviewPacket`
- `ReviewState`, `ReviewTargets`, `ReviewerAuditReceipt`, `ReviewerResponseShape`, `ReviewerRuntimeContract`, `RoleCapability`, `RoleCapabilityRegistry`, `RoleCardinalityViolation`
- `RoleCommandEnvelope`, `RoleCreationAction`, `RoleGuard`, `RoleInstructionCard`, `RoleReviewAssignmentLifecycle`, `RoleReviewReceipt`, `RoleReviewTimeout`, `RoleRoundViolation`
- `RunRecord`, `RuntimeErrorReceipt`, `RuntimeStateIgnorePostureGuard`, `RuntimeTruthSnapshot`, `RustAuditReport`, `SchemaMigrationSpine`, `SecurityReport`, `SessionActivityEntry`
- `SessionActivityLog`, `SessionCachePacket`, `SessionLease`, `SessionLivenessReconciler`, `SessionLivenessSignal`, `SessionOrientationPacket`, `SessionPosture`, `SessionStatusProjection`
- `SessionTerminationPolicy`, `SkillCompatibilityValidator`, `SkillLoadReceipt`, `SkillManifest`, `SliceClosureEvent`, `StaleFindingReceipt`, `StaleProofRef`, `StalledQueueFinding`
- `SubagentAuthorityViolation`, `SystemMapSnapshot`, `SystemPicture`, `TagReceipt`, `TaskCompleteDecision`, `TaskProducedTtlEvidence`, `TaskStartedAdrPrecedentLinkingGuard`, `ToggleReceipt`
- `TokenOptimizationDogfoodReceipt`, `TransitionContract`, `TransitionViolation`, `TypedAction`, `TypedGateFailure`, `TypedNamespaceCompositionGuard`, `TypedOutputHumanSummary`, `TypingCompatUtility`
- `WorkPublicationLedger`, `WorkPublicationLedgerEvent`, `WorkPublicationLedgerHeader`, `WorkflowAdapter`, `WorkflowStep`, `WorktreeBaseline`
...
### 2 BROKEN seams observed
1. `control_plane_daemons.py` — no factory/registry; can't add daemon without editing 3+ subsystems
2. `review_state_refresh_support.py` — refresh authority defined but scattered writers bypass it

Resolved 2026-05-01: `dev/scripts/checks/mutation_outcome_parse.py` no longer blocks `devctl triage`; the shim now works in package-import and direct-script modes.
...
dashboard_builders (11 sections)
    ↓
dashboard_snapshot_authority (DashboardSnapshot v3)
    ↓
dashboard / claude-loop / mobile-status
    ↓
operator_console (PyQt6, ~23.5k LOC, keeps lane panels and attaches DashboardSnapshot v3)
```
...
`state/` (43 files, 5.9k LOC): bridge/review/sessions/snapshots/core/activity/presentation/repo/jobs. No subprocess calls — reads artifacts directly. It still builds lane-specific `OperatorConsoleSnapshot` panels, but also attaches the shared `DashboardSnapshot` v3 contract so desktop, CLI, and mobile consumers read the same typed sections.

### Dashboard-ready commands
`dashboard`, `claude-loop`, `phone-status`, `mobile-status`, `status`, `startup-context`, `control-plane read-model` (internal).
...
| 3 | Dashboard snapshot builders: CLI dashboard + `mobile-status` + Operator Console | `DashboardSnapshot` v3 now routes CLI/mobile/desktop dashboard sections through `dashboard_snapshot_authority`; `system-picture` remains a separate external-review reducer |
...
Provider ids are adapters, not authority. Packets, slash adapters, `/develop`,
router lanes, startup/session-resume, dashboards, and VCS gates must route work
through actor role, exact session id when a live session is intended, and
capability grants from `CollaborationSession` / `AuthoritySnapshot`. `codex`,
`claude`, `cursor`, and future providers only name the runtime used to reach
that actor.
