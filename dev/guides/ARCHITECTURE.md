...
### Process-orphan prevention contract (hardening requirement)

- Runtime and test harness flows must exit with **zero detached VoiceTerm/cargo descendants** attributable to the current repo/session.
- Add a scoped orphan watchdog for stale `cargo test --bin voiceterm` and detached `target/debug/deps/voiceterm-*` processes (`PPID=1`) with owner/session metadata checks before reap.
- Expose an auditable operator command (`devctl` process-health/report + optional cleanup mode) so stale process state is visible before and after runtime/test work.
- Add post-run guard checks in local CI/dev bundles so lanes fail when orphan counts exceed policy thresholds.
