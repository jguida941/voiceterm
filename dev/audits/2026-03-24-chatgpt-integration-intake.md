...
## Final 8-Agent Architecture Alignment Audit (2026-03-25)

Deep audit of convo.md ideas vs actual architecture. Found 7 items not
yet captured in active plan docs, plus confirmed 4 of 5 feedback loops
are closed and identified specific wiring points where ZGraph could
drive decisions (~170-260 lines total).

**Review disposition (2026-03-25, follow-up)**: do **not** promote this
section as-is. A second repo-grounded recheck showed the "7 items not yet
captured" claim is too broad. Multi-agent coordination is already covered in
`AGENTS.md` plus the active `review_channel` / `continuous_swarm` plan stack;
compiler-pass framing already lives in `ai_governance_platform.md`; graph
consumer expansion is already the active `MP-377` graph lane; and the
self-hosting claim overstates current hard-limit drift (confirmed local check:
`check_rust_best_practices.py` is over its 650-line hard limit, but not "3
guard files"). The narrower items that did survive review have already been
promoted into active plan state: raw interactive bootstrap enforcement,
generated graph normalization/compaction, shared parser/AST reuse, cross-file
probe pre-analysis, and explicit `system-picture` plan-status/progress
percentages.

### NOT IN PLANS: Multi-Agent Audit Protocol

The user asked (convo line 5667): "I need you to audit this with multiple
agents — one audits then gives tasks to seven others." No plan doc
describes how agents coordinate findings, escalate to global context,
or merge conflicting results. Current plans assume single-agent flow.

**Needed**: `multi_agent_audit_protocol.md` or section in
`ai_governance_platform.md` covering: intake SOP per agent, escalation
rules (local → global), consensus model, finding merge rules.

### NOT IN PLANS: Deferred Decision Guard Pattern

The `InlineOrHelperGuard` pseudocode (convo lines 5477-5499) shows
local analysis + ZGraph global analysis → deferred finalization. No
guard in the system uses this pattern except `check_function_duplication`
(which does two-pass). Should be documented as a reusable guard design
pattern in `portable_code_governance.md`.

### NOT IN PLANS: Deterministic Precomputation Layer

Convo line 5122: "precompute deterministic outcomes before reasoning."
Before AI traverses the graph, all Boolean guard outcomes that are
already determined should be resolved. Reduces the decision surface AI
sees. No pre-reasoning precomputation pass exists.

### NOT IN PLANS: Compiler Optimization Patterns Guide

The convo maps 5 compiler techniques to the system (DCE → dead nodes,
inlining → merge helpers, constant folding → precompute checks, CSE →
parse caching, dead store → write-only artifacts). No architecture doc
captures this mental model. Should be a guide:
`dev/guides/AI_COMPILER_PATTERNS.md`.

### CONFIRMED: 4 of 5 Feedback Loops Are Closed

| Loop | Status |
|---|---|
| Probe → fix → probe knows (via allowlist) | **CLOSED** |
| Governance verdict → next probe excludes | **CLOSED** |
| Ralph fix → next attempt learns | **OPEN** (ralph-report.json not read by next attempt) |
| Watchdog failure → context escalation | **CLOSED** |
| Quality feedback → decision constraints | **CLOSED** |

Only the Ralph attempt-to-attempt loop is open. All others flow
end-to-end through allowlists, context escalation, and decision
packets.

### CONFIRMED: ZGraph Has 5 Decision Wiring Points (~170-260 lines)

| Component | Graph Data Available | Used for Decisions? | Lines to Wire |
|---|---|---|---|
| ralph_prompt.py | Temperature in matched nodes | No — markdown only | ~25 |
| autonomy_loop.py | Graph not loaded | No — time/count caps only | ~50 |
| triage/enrich.py | Graph not queried | No — static severity map | ~65 |
| quality_backlog/priorities.py | fan_in/fan_out in metadata | No — static weights | ~40 |
| WorkIntakePacket | Graph not loaded | No — text matching only | ~30 |

Total: ~210 lines to make ZGraph a decision engine, not just a renderer.

### CONFIRMED: Self-Hosting Is 80% Complete

| Aspect | Status |
|---|---|
| Guards run on own code | Quick passes; CI absolute finds 8 violations |
| 3 guard files exceed own limits | HARD violations untracked |
| Plan docs governed | YES — 3+ guards scan dev/active/ |
| CLAUDE.md generated by render-surfaces | YES |
| Differential scanning catches violations | NO — needs --absolute mode |

Key gap: `check_rust_best_practices.py` at 706 lines exceeds the 650
hard limit. The governance system doesn't fully pass its own rules.

### CONFIRMED: Portable Readiness Is ~60-70%

Core engine works externally (ci-cd-hub proven: 159 hints across 509
files). Main blockers:
- 81 voiceterm-specific hardcoded paths in check scripts
- Review-channel not opt-in (mandatory for full flow)
- No portable startup-authority or work-intake abstraction
- No discovery commands (guard-explain, which-tests)

### CONFIRMED: Proof/Explanation Gaps

| Artifact | Has Proof? | Gap |
|---|---|---|
| Guard violations | YES — reason + guidance + policy | — |
| Probe guidance | YES — decision_mode + rationale | rationale field optional |
| Autonomy stop | YES — reason codes | No triage classification |
| Governance review | PARTIAL — verdict only | No explanation field |
| Context graph query | MINIMAL — match count only | No "why matched" |
| Ralph fix | PARTIAL — status only | No reasoning chain |

Missing: unified `DecisionTrace` artifact linking guard → graph → evidence
→ before/after → outcome (tracked in MASTER_PLAN.md:3153 but not built).

### CONFIRMED: Operator Visibility Gaps

- NO unified finding→guidance→fix→verdict chain view
- NO guidance_id populated in production (field exists, 0 rows use it)
- NO historical guard flakiness trends (current only)
