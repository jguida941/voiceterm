...
1. **Inventory**: emit typed `TopologyHardcodeInventory` at `dev/state/topology_hardcode_inventory.jsonl` — one row per location in `dev/scripts/devctl/runtime/*.py` AND `dev/scripts/devctl/governance/instruction_boot_card.py` (the boot-card template source) that hardcodes `"codex"` / `"claude"` / `"cursor"` literals or count-based topology naming. Expected ~28 runtime file rows + N boot-card-template rows per audit. Inventory only — NO refactor in this phase. **PROJECTION-PARITY ENTRY (operator-mandated 2026-05-18 ~18:00 EDT)**: empirical proof of the hardcoding bug — `AGENTS.md` line 15 says `--role reviewer --actor codex` while `CLAUDE.md` line 15 says `--role implementer --actor claude`. Both are generated by the same `render-surfaces` from `instruction_boot_card.py` but hardcode different actor/role pairs per provider-projection. Any agent can play any role per cached-hammock P58.5; the boot card should parameterize OR enumerate all combinations, NOT hardcode per-projection. Both projections must derive byte-equal content (modulo `surface_id` marker) from the same typed source. This is the SOURCE of the runtime hardcoding family — fixing it (Phase 6) cascades through the 28 runtime sites.
