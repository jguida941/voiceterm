...
mod tests {
    ...
    #[test]
    fn progress_eta_requires_strict_bounds_and_elapsed_time() {
        let mut zero_progress = Progress::new(0.0);
        zero_progress.elapsed_secs = 10.0;
        assert!(zero_progress.eta_secs().is_none());

        let mut full_progress = Progress::new(1.0);
        full_progress.elapsed_secs = 10.0;
        assert!(full_progress.eta_secs().is_none());

        let mut no_elapsed = Progress::new(0.5);
        no_elapsed.elapsed_secs = 0.0;
        assert!(no_elapsed.eta_secs().is_none());
    }
    ...
    #[test]
    fn format_bouncing_bar_exact_positions_across_bounce_cycle() {
        assert_eq!(format_bouncing_bar(0, 10, Theme::None), "[====      ]");
        assert_eq!(format_bouncing_bar(5, 10, Theme::None), "[     ==== ]");
        assert_eq!(format_bouncing_bar(6, 10, Theme::None), "[      ====]");
        assert_eq!(format_bouncing_bar(7, 10, Theme::None), "[     ==== ]");
        assert_eq!(format_bouncing_bar(8, 10, Theme::None), "[    ====  ]");
    }

    #[test]
    fn format_bouncing_bar_handles_tight_widths() {
        assert_eq!(format_bouncing_bar(0, 0, Theme::None), "[]");
        assert_eq!(format_bouncing_bar(5, 1, Theme::None), "[=]");
        assert_eq!(format_bouncing_bar(9, 3, Theme::None), "[===]");
    }
