...
#
# Usage:
#   ./scripts/tests/measure_latency.sh               # Interactive mode, 10 samples
#   ./scripts/tests/measure_latency.sh --synthetic   # Synthetic audio mode
#   ./scripts/tests/measure_latency.sh --voice-only  # Measure voice pipeline only
#   ./scripts/tests/measure_latency.sh --ci-guard    # CI-friendly synthetic latency guardrails
...
if [[ "$MODE" == "ci-guard" ]]; then
  ...
  echo ""
  read -r -p "Press Enter when ready to start measurements..."
  echo ""
...
echo "2. Include relevant results in an issue or release note"
