...
**Speaker background** (operator-researched 2026-05-13T~06:35Z): Tim McGilchrist is an Australian functional-programming engineer. Active in the FP-SYD / YOW! / Compose conference circuit. Background in OCaml, Haskell, Idris, Erlang, Lisp, distributed systems, compiler/runtime infrastructure. Previously at **Ambiata** (Haskell/OCaml ML + big-data infra), currently **Principal Software Engineer at Tarides** (OCaml multicore CI/ops infrastructure). Talk archive: tmcgilchrist.github.io. NOT a celebrity researcher like SPJ or Edwin Brady, but firmly inside the serious typed-systems engineering world. The talk is NOT "business workflow state machines" — it is specifically *"Can we encode legal transitions into the type/lifecycle system itself so invalid operations become impossible or explicitly rejected?"* That is why the talk maps 1:1 onto the operator's governed-receipts / lifecycle-gates / transition-legality / semantic-graph / typestate-governance frame: **fundamentally the same category of problem**, attacked from the dependent-types side rather than the runtime-governance side.
