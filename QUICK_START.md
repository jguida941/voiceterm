...
If `codex-voice` is not found, add the install dir to PATH (default: `~/.local/bin`), or set
`CODEX_VOICE_INSTALL_DIR` to a writable folder before running `./install.sh`.
