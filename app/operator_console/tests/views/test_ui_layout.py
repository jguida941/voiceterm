...
def _blank_snapshot():
    from app.operator_console.state.models import OperatorConsoleSnapshot
...
class NavTabsTests(unittest.TestCase):
    ...
    def test_nav_tabs_has_four_pages(self) -> None:
        window = _make_window()
    ...
    def test_home_keeps_toolbar_actions_off_page(self) -> None:
        window = _make_window()
        self.assertIsNone(window.home_workspace.start_swarm_button.parent())
...
class MonitorTabsTests(unittest.TestCase):
    ...
    def test_monitor_tabs_has_five_panels(self) -> None:
        window = _make_window()
        self.assertEqual(window._monitor_tabs.count(), 5)
    ...
    def test_monitor_tab_labels_match_session_first_order(self) -> None:
        ...
        self.assertEqual(
            labels,
            ["Codex Session", "Claude Session", "Bridge", "Launcher Output", "Diagnostics"],
        )
    ...
    def test_reveal_command_output_selects_monitor_output_tab(self) -> None:
        ...
        self.assertEqual(window._monitor_tabs.currentIndex(), 3)
...
class ActivityPageTests(unittest.TestCase):
    ...
    def test_activity_keeps_toolbar_actions_off_page(self) -> None:
        window = _make_window()
        self.assertIsNone(window.activity_workspace.activity_start_swarm_button.parent())
    ...
    def test_switching_to_technical_mode_refreshes_without_crash(self) -> None:
        ...
        from app.operator_console.state.presentation_state import AnalyticsViewState
...
class LaneStructureTests(unittest.TestCase):
    ...
    def test_busy_state_updates_action_buttons(self) -> None:
        window = _make_window()
        window._set_command_controls_busy(True, label="Swarm...")
        ...
        self.assertEqual(window.launch_live_button.text(), "Swarm...")
        self.assertEqual(window.home_workspace.start_swarm_button.text(), "Swarm...")
        self.assertEqual(window.activity_workspace.activity_start_swarm_button.text(), "Swarm...")
        ...
        self.assertEqual(window.home_workspace.start_swarm_button.text(), "Start Swarm")
        self.assertEqual(window.activity_workspace.activity_start_swarm_button.text(), "Start Swarm")

    def test_start_swarm_uses_json_preflight_command(self) -> None:
        from app.operator_console.state.command_builder import build_launch_command
