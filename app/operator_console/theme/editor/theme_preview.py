...
if _PYQT_AVAILABLE:
    ...
    from .motion_preview import ThemeMotionPlayground
    from ...views.agent_detail import DiffHighlighter
    from ...views.approval_panel import ApprovalQueuePanel
    from ...views.shared.widgets import KeyValuePanel, ProviderBadge, StatusIndicator
    ...
    class ThemePreview(QWidget):
        ...
        def __init__(self, parent: QWidget | None = None) -> None:
            ...
            layout.addLayout(self._build_preview_row(self._build_diff_card(), self._build_motion_card()))
            layout.addLayout(self._build_preview_row(self._build_style_card(), self._build_empty_error_card()))
            layout.addStretch(1)
        ...
        def set_preview_theme(
            ...
            if motion is not None:
                self._motion_playground.set_motion_settings(motion)
        ...
        def _build_style_card(self) -> QFrame:
            ...
            title = QLabel("Component Styles")
            title.setObjectName("CardAgentName")
            subtitle = QLabel("Button, toolbar, tab, and surface families under the live theme contract")
            subtitle.setObjectName("CardLaneLabel")
            subtitle.setWordWrap(True)
            layout.addWidget(title)
            layout.addWidget(subtitle)
            ...
            soft = QPushButton("Soft Fill")
            soft.setProperty("accentRole", "primary")
            outline = QPushButton("Outline")
            ghost = QPushButton("Ghost")
            ghost.setObjectName("SmallActionButton")
            style_buttons.addWidget(soft)
            style_buttons.addWidget(outline)
            style_buttons.addWidget(ghost)
            layout.addLayout(style_buttons)

            monitor_tabs = QTabWidget()
            monitor_tabs.setObjectName("MonitorTabs")
            monitor_tabs.tabBar().setObjectName("MonitorTabBar")
            monitor_tabs.addTab(QLabel("Analytics-focused"), "Dashboard")
            monitor_tabs.addTab(QLabel("Terminal-first"), "Monitor")
            monitor_tabs.setCurrentIndex(0)
            layout.addWidget(monitor_tabs)
            return card
        ...
        def _build_motion_card(self) -> QFrame:
            ...
            title = QLabel("Motion Playground")
            title.setObjectName("LaneAgentName")
            subtitle = QLabel("Preview front/back swaps and pulse behavior with the active motion settings")
            subtitle.setObjectName("CardDetailLabel")
            subtitle.setWordWrap(True)
            layout.addWidget(title)
            layout.addWidget(subtitle)

            self._motion_playground = ThemeMotionPlayground()
            layout.addWidget(self._motion_playground)
            return card
