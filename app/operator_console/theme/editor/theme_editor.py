...
def _safe_int(value: object, fallback: int = 0) -> int:
    """Convert *value* to int, returning *fallback* for non-numeric inputs.

    Prevents ValueError when malformed theme JSON stores strings like
    ``"fast"`` in fields that expect integer milliseconds.
    """
    try:
...
def _sync_control_registry(
    registries: Iterable[dict[str, QWidget]],
    state_values: dict[str, str],
    *,
    reference_colors: tuple[str, str] | None = None,
) -> None:
    """Synchronize one or more widget registries from engine state values.

    Handles blockSignals + isinstance dispatch for QSpinBox, QLineEdit,
    QCheckBox, QComboBox, and ColorPickerButton.  When *reference_colors*
    is provided (foreground, background), ColorPickerButton swatches get
    their contrast reference set before the color value is applied.
    """
    for registry in registries:
        for name, control in registry.items():
            value = state_values.get(name, "")
            if reference_colors is not None and hasattr(control, "set_reference_colors"):
                control.set_reference_colors(
                    foreground_color=reference_colors[0],
                    background_color=reference_colors[1],
                )
            if hasattr(control, "color_changed"):
                # ColorPickerButton: set the color directly (no blockSignals)
                from ..runtime.theme_engine import BUILTIN_PRESETS

                fallback = BUILTIN_PRESETS["Codex"].colors.get(
                    name,
                    BUILTIN_PRESETS["Codex"].colors["accent"],
                )
                control.color = value or fallback
                continue
            if isinstance(control, QSpinBox):
                control.blockSignals(True)
                control.setValue(_safe_int(value, control.minimum()))
                control.blockSignals(False)
            elif isinstance(control, QCheckBox):
                control.blockSignals(True)
                control.setChecked(str(value).lower() == "true")
                control.blockSignals(False)
...
if _PYQT_AVAILABLE:
    from ..config.theme_components import COMPONENT_CATEGORIES, COMPONENT_SPECS
    from .theme_controls import ColorPickerButton
    from ..runtime.theme_engine import BUILTIN_PRESETS, ThemeEngine, get_engine
    from ..io.theme_file_ops import (
        apply_overlay_text,
        apply_pasted_json,
        apply_pasted_qss,
        export_json_file,
        export_overlay_style_pack,
    ...
    from ..config.theme_motion import MOTION_CATEGORIES, MOTION_SPECS
    from .theme_preview import ThemePreview
    from ..config.theme_tokens import TOKEN_CATEGORIES, TOKEN_SPECS
    ...
    class ThemeEditorDialog(QDialog):
        ...
        def __init__(self, parent: QWidget | None = None) -> None:
            ...
            self._component_controls: dict[str, QWidget] = {}
            self._motion_controls: dict[str, QWidget] = {}
            self._quick_color_controls: dict[str, ColorPickerButton] = {}
            self._quick_token_controls: dict[str, QWidget] = {}
            self._quick_component_controls: dict[str, QWidget] = {}
            self._quick_motion_controls: dict[str, QWidget] = {}
        ...
        def _build_editor_workbench(self) -> QWidget:
            ...
            pages = (
                ...
                ("Motion", self._build_motion_page()),
        ...
        def _build_guide_page(self) -> QWidget:
            ...
            limits_text = QLabel(
                ...
                "tokens, component style families, JSON import/export, and QSS color import. "
                "Motion settings are previewable in the editor, but they are not yet a "
                "whole-app runtime contract across every live Operator Console surface."
            )
        ...
        def _build_surfaces_page(self) -> QWidget:
            page = self._build_color_groups_page(
                intro_text=(
        ...
        def _build_components_page(self) -> QWidget:
            ...
            intro = QLabel(
                "Pick global component families for corners, borders, cards, inputs, buttons, and tabs."
            )
            intro.setObjectName("MutedLabel")
            intro.setWordWrap(True)
            layout.addWidget(intro)

            for category_name, keys in COMPONENT_CATEGORIES.items():
                group = self._make_editor_group(category_name)
                group_layout = QFormLayout(group)
                for key in keys:
                    spec = COMPONENT_SPECS[key]
                    control = self._make_component_control(
                        key,
                        spec,
                        registry=self._component_controls,
                    )
                    group_layout.addRow(QLabel(spec.label), control)
                layout.addWidget(group)
        ...
        def _build_motion_page(self) -> QWidget:
            ...
            intro = QLabel(
                "Author motion intentionally. These settings drive the editor playground now and provide the runtime contract for wider app adoption."
            )
            intro.setObjectName("MutedLabel")
            intro.setWordWrap(True)
            layout.addWidget(intro)

            for category_name, keys in MOTION_CATEGORIES.items():
                group = self._make_editor_group(category_name)
                group_layout = QFormLayout(group)
                for key in keys:
                    spec = MOTION_SPECS[key]
                    control = self._make_motion_control(
                        key,
                        spec,
                        registry=self._motion_controls,
                    )
                    group_layout.addRow(QLabel(spec.label), control)
                layout.addWidget(group)

            note_group = self._make_editor_group("Where Motion Applies")
            note_layout = QVBoxLayout(note_group)
            note = QLabel(
                "The preview playground uses these settings for front/back swaps and pulse feedback today. The broader console can adopt the same contract surface-by-surface without inventing new theme keys later."
            )
            note.setObjectName("CardDetailLabel")
        ...
        def _build_theme_authoring_notes_group(self) -> QWidget:
            ...
            notes_text = QLabel(
                ...
                "theme contract including colors, sizing tokens, component style settings, "
                "and motion settings. Pasted QSS still maps colors onto that same contract "
        ...
        def _build_json_theme_group(self) -> QWidget:
            ...
            self._json_paste.setPlaceholderText(
                ...
                '  "motion": { "page_transition": "fade", "page_transition_ms": 180 }\n'
                '}'
            )
        ...
        def _build_quick_tune_page(self) -> QWidget:
            ...
            component_group = self._make_editor_group("Component Modes")
            component_layout = QFormLayout(component_group)
            for key in _QUICK_COMPONENT_KEYS:
                spec = COMPONENT_SPECS[key]
                control = self._make_component_control(
                    key,
                    spec,
                    registry=self._quick_component_controls,
                )
                component_layout.addRow(QLabel(spec.label), control)
            layout.addWidget(component_group)

            motion_group = self._make_editor_group("Motion")
            motion_layout = QFormLayout(motion_group)
            for key in _QUICK_MOTION_KEYS:
                spec = MOTION_SPECS[key]
                control = self._make_motion_control(
                    key,
                    spec,
                    registry=self._quick_motion_controls,
                )
                motion_layout.addRow(QLabel(spec.label), control)
        ...
        def _build_coverage_page(self) -> QWidget:
            ...
            editable_text = QLabel(
                ...
                "Components: button families, toolbar chrome, input treatment, border density, surface tone, and tab styles.\n"
                "Motion: previewable page fades and pulse feedback.\n"
            ...
            pending_text = QLabel(
                "Overlay round-trip still intentionally stays narrow: shared base-theme import/export comes first, while broader Rust component/motion parity remains blocked until that cross-surface contract is proven stable."
            )
            ...
            workflow_text = QLabel(
                "Quick Tune is for high-signal adjustments you make repeatedly. The left editor pages stay the authority for full-surface editing across colors, metrics, components, and motion. Preview is secondary when the live Operator Console window is already open on another display."
            )
        ...
        def _make_component_control(
            ...
            *,
            ...
            combo = QComboBox()
            for value, label in getattr(spec, "options", ()):
                combo.addItem(label, value)
            combo.currentIndexChanged.connect(
                lambda _index, name=key, control=combo: self._on_component_changed(
                    name,
            ...
            *,
            ...
            if value_type == "bool":
                ...
                checkbox.toggled.connect(
                    lambda checked, name=key: self._on_motion_changed(name, checked)
                )
            ...
            if value_type == "int":
                ...
                spin.valueChanged.connect(
                    lambda value, name=key: self._on_motion_changed(name, value)
                )
                registry[key] = spin
                return spin
            ...
            for value, label in getattr(spec, "options", ()):
                combo.addItem(label, value)
            combo.currentIndexChanged.connect(
                lambda _index, name=key, control=combo: self._on_motion_changed(
                    name,
                    str(control.currentData()),
                )
            )
        ...
        def _on_component_changed(self, name: str, value: str) -> None:
            if self._updating_controls:
                return
            self._engine.set_component(name, value)
            self._engine.save_current()

        def _on_motion_changed(self, name: str, value: int | str | bool) -> None:
            if self._updating_controls:
                return
            self._engine.set_motion(name, value)
            self._engine.save_current()
        ...
        def _sync_from_engine(self) -> None:
            ...
            try:
                ...
                ref = (foreground_color, background_color)

                _sync_control_registry(
                    (self._color_controls, self._quick_color_controls),
                    state.colors,
                    reference_colors=ref,
                )
                _sync_control_registry(
                    (self._token_controls, self._quick_token_controls),
                    state.tokens,
                )
                _sync_control_registry(
                    (self._component_controls, self._quick_component_controls),
                    state.components,
                )
                _sync_control_registry(
                    (self._motion_controls, self._quick_motion_controls),
                    state.motion,
                )
                ...
                self._preview.set_preview_theme(state.colors, state.motion)
                ...
                self._selection_detail_label.setText(
                    "Buttons: "
                    f"{state.components.get('button_style', 'soft-fill')} | "
                    "Toolbar: "
                    f"{state.components.get('toolbar_button_style', 'chip')} | "
                    "Tabs: "
                    f"{state.components.get('nav_tab_style', 'underline')} / "
                    f"{state.components.get('monitor_tab_style', 'boxed')} | "
                    "Motion: "
                    f"{state.motion.get('page_transition', 'fade')} "
                    f"{state.motion.get('page_transition_ms', '160')}ms"
                )
