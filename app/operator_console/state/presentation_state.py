...
from .analytics_snapshot import RepoAnalyticsSnapshot
from .models import AgentLaneData, OperatorConsoleSnapshot
from .phone_status_snapshot import PhoneControlSnapshot
from .readability import audience_mode_label, resolve_audience_mode
from .repo_state import RepoStateSnapshot
...
def _present_lanes(snapshot: OperatorConsoleSnapshot) -> list[AgentLaneData]:
    return [
        lane
        for lane in (snapshot.codex_lane, snapshot.operator_lane, snapshot.claude_lane)
        if lane is not None
    ]
