...
from .lane_builder import build_claude_lane, build_codex_lane, build_operator_lane
from .models import OperatorConsoleSnapshot
...
from .session_builder import build_claude_session_text, build_codex_session_text
from .session_trace_reader import load_live_session_trace
...
def build_operator_console_snapshot(
    ...
    *,
    ...
    codex_lane = build_codex_lane(
        sections, metadata.last_codex_poll, metadata.last_worktree_hash
    )
    claude_lane = build_claude_lane(sections)
    operator_lane = build_operator_lane(
        sections, pending_approvals, resolved_review_state
    )
    codex_session_text = build_codex_session_text(
        live_trace=load_live_session_trace(
            repo_root,
            provider="codex",
    ...
    claude_session_text = build_claude_session_text(
        live_trace=load_live_session_trace(
            repo_root,
            provider="claude",
            review_full_path=resolved_review_full,
    ...
    return OperatorConsoleSnapshot(
        ...
        codex_session_text=codex_session_text,
        claude_session_text=claude_session_text,
