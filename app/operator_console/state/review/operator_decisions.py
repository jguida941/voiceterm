...
from ...collaboration.context_pack_refs import (
...
def _render_operator_decision_markdown(payload: dict[str, object]) -> str:
    ...
    if isinstance(evidence_refs, list) and evidence_refs:
        ...
        for item in evidence_refs:
            lines.append(f"- {item}")
    context_pack_refs = payload.get("context_pack_refs")
    if isinstance(context_pack_refs, list) and context_pack_refs:
        lines.append("")
        lines.append("## Context Pack Refs")
        for line in context_pack_ref_lines(parse_context_pack_refs(context_pack_refs)):
            lines.append(f"- {line}")
    return "\n".join(lines)
...
def approval_request_from_payload(payload: object) -> ApprovalRequest:
    ...
    context_pack_refs = parse_context_pack_refs(payload.get("context_pack_refs"))
