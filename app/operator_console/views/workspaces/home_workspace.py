...
from ...state.presentation.presentation_state import SystemBannerState
...
from ..shared.widgets import compact_display_text
...
class HomeWorkspace(QWidget if _PYQT_AVAILABLE else object):
    ...
    def _init_workflow_launchpad(self) -> None:
        """Create the shared launchpad widget and expose stable aliases."""
        self.workflow_launchpad = WorkflowLaunchpad()
        self.workflow_selector = self.workflow_launchpad.workflow_selector
        self.run_loop_button = self.workflow_launchpad.run_loop_button
        self.audit_button = self.workflow_launchpad.audit_button
        self.home_dry_run_button = self.workflow_launchpad.dry_run_button
        self.start_swarm_button = self.workflow_launchpad.launch_review_button
        ...
        self.workflow_steps_label = self.workflow_launchpad.workflow_steps_label
    ...
    def _build_nav_card(self) -> QFrame:
        """Visible launcher card for the read-first home surface."""
        card = QFrame()
        card.setObjectName("LaneCard")
        layout = QVBoxLayout(card)
        layout.setContentsMargins(16, 12, 16, 12)
        ...
        self._nav_intro_label = QLabel(
            "Pick the markdown plan here, then run the loop or launch the review lanes without hunting through the toolbar."
        )
        ...
        nav_title.setObjectName("SectionHeaderLabel")
        layout.addWidget(nav_title)

        self._toolbar_hint_label = QLabel(
            "Raw command output stays in Monitor. Reports gives the readable digest."
        )
        ...
        nav_grid.addWidget(self.dashboard_button, 0, 0)
        nav_grid.addWidget(self.monitor_button, 0, 1)
        nav_grid.addWidget(self.activity_button, 1, 0)
        nav_grid.addWidget(self.theme_button, 1, 1)
        nav_grid.addWidget(self.guide_button, 2, 0, 1, 2)
        layout.addLayout(nav_grid)

        self._page_hint_label = QLabel(
            "Dashboard shows lane state. Monitor shows raw launcher output. Reports stages readable summaries."
        )
    ...
    def set_audience_mode(self, mode: str) -> None:
        ...
        self._nav_intro_label.setText(
            "Choose the markdown scope here, then run loop/audit/review actions without switching contexts."
            if technical_mode
            else "Pick the plan here first, then act from the visible buttons instead of hunting through the toolbar."
        )
        self._toolbar_hint_label.setText(
            "Monitor = raw launcher output. Reports = readable digest. Toolbar stays available for low-level actions."
            if technical_mode
            else "Monitor holds raw output. Reports holds the readable digest."
        )
        self._page_hint_label.setText(
            "Dashboard = lane state. Monitor = raw logs. Reports = digest plus staged prompt."
            if technical_mode
            else "Dashboard shows lane state, Monitor shows raw logs, and Reports shows the readable summary."
        )
