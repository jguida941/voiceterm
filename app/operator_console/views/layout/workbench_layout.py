"""Workbench layout helpers for the Operator Console layout package.
...
def build_workbench_content(window: object) -> QWidget:
    ...
    root.setSpacing(8)
    root.addWidget(window.workflow_header_bar)
    root.addWidget(workbench_tabs, stretch=1)
    root.addWidget(window.workflow_timeline_footer)
    ...
    workbench_tabs.currentChanged.connect(
        lambda _index: _persist_layout_state_if_available(window)
    )
    session_splitter.splitterMoved.connect(
        lambda _pos, _index: _persist_layout_state_if_available(window)
    )
    terminal_splitter.splitterMoved.connect(
        lambda _pos, _index: _persist_layout_state_if_available(window)
    )
    monitor_tabs = getattr(window, "_monitor_tabs", None)
    if monitor_tabs is not None:
        monitor_tabs.currentChanged.connect(
            lambda _index: _persist_layout_state_if_available(window)
        )
...
def apply_workbench_preset(
    ...
    *,
    ...
    if announce:
        ...
        _persist_layout_state_if_available(window)
...
def _build_session_page(window: object) -> QSplitter:
    ...
    splitter.addWidget(window._build_session_surface("codex"))  # type: ignore[attr-defined]
    splitter.addWidget(window._build_session_surface("cursor"))  # type: ignore[attr-defined]
    splitter.addWidget(window._build_center_spine(include_approvals=False))  # type: ignore[attr-defined]
    splitter.addWidget(window._build_session_surface("claude"))  # type: ignore[attr-defined]
...
def _build_log_monitor_tabs(window: object) -> QTabWidget:
    ...
    monitor_panels = (
        ("timeline", window.timeline_panel, "Timeline"),
...
def _build_stats_page(window: object) -> QWidget:
    ...
    for card in (
        ...
        window.workbench_cursor_card,
    ):
        card.setMaximumHeight(140)

    grid.addWidget(window.workbench_codex_card, 0, 0)
    grid.addWidget(window.workbench_operator_card, 0, 1)
    grid.addWidget(window.workbench_cursor_card, 0, 2)
    grid.addWidget(window.workbench_claude_card, 0, 3)
    grid.addWidget(
        window._wrap_in_card(  # type: ignore[attr-defined]
...
def _persist_layout_state_if_available(window: object) -> None:
    persist = getattr(window, "_persist_layout_state", None)
    if callable(persist):
        persist()
