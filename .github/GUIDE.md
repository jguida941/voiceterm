...
| `.github/workflows/security_guard.yml` | Scheduled, manual, and dependency/security-file push/PR | RustSec advisory policy gate + artifact report | `cd src && (cargo audit --json > ../rustsec-audit.json || true)` then `python3 dev/scripts/checks/check_rustsec_policy.py --input rustsec-audit.json --min-cvss 7.0 --fail-on-kind yanked --fail-on-kind unsound --allowlist-file dev/security/rustsec_allowlist.md` | Required (targeted) + Optional (scheduled/manual) |
