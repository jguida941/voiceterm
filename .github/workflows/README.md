...
| `tooling_control_plane.yml` | Main tooling/governance gate. | Push/PR on tooling/docs/governance/workflow paths. | Devctl unit tests, script integrity, strict docs/governance checks, strict-warning hygiene mode, architecture-surface sync, workflow-shell hygiene, workflow action-pinning, bundle/workflow parity, IDE/provider isolation, compat-matrix schema/smoke, naming consistency guards, Rust test-shape + runtime panic-policy guards, remediation scaffold on failure. | `python3 dev/scripts/devctl.py docs-check --strict-tooling` |
