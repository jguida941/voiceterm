...
| `ios_ci.yml` | iOS/Swift quality lane. | Push/PR when `app/ios/` paths or the iOS CI workflow file change. | Swift package build (`swift build`), Swift package tests (`swift test`) covering mobile relay snapshot and projection bundle tests, Xcode app build for iOS Simulator (`xcodebuild build` with code-signing disabled). | `cd app/ios/VoiceTermMobile && swift test` |
...
| `adopter_portability.yml` | Proves portable governance bootstrap on non-VoiceTerm adopter fixtures. | Push/PR when governance tooling, architecture probes, adopter fixtures, or this workflow change. | Copies greenfield and existing-plan Python adopter fixtures, initializes them as standalone git repos, runs `devctl governance-bootstrap --target-repo`, and asserts the typed `AdopterPortabilityValidation` contract contains `ProjectGovernance`, `MasterPlan`, and `PlanRow` without VoiceTerm assumptions. | `python3 dev/scripts/devctl.py governance-bootstrap --target-repo /tmp/copied-repo --force-starter-policy --format md` |
...
| `tooling_control_plane.yml` | Main tooling/governance gate. | Push/PR on tooling/docs/governance/workflow paths. | Devctl unit tests, operator console tests, script integrity, mypy type-check (advisory/non-blocking), strict docs/governance checks, strict-warning hygiene mode, plan-index commit continuity, architecture-surface sync, workflow-shell hygiene, workflow action-pinning, bundle/workflow parity, platform-contract sync + closure guards, IDE/provider isolation, compat-matrix schema/smoke, naming consistency guards, Rust test-shape + serde-compatibility + runtime panic-policy guards, facade-wrappers + god-class + nesting-depth + parameter-count + python-dict-schema + python-global-mutable + function-duplication + structural-similarity guards, remediation scaffold on failure. | `python3 dev/scripts/devctl.py docs-check --strict-tooling` |
...
| `pre_commit.yml` | Runs pre-commit hooks on Python and config files. | Push/PR when Python, YAML, TOML, or pre-commit config paths change. | Trailing whitespace, end-of-file fixer, YAML/TOML checks, large file check, merge conflict markers, private key detection, ruff lint + format, yamllint. | `pre-commit run --all-files` |
...
| Workflow file | What it does | When it runs | Main checks/actions | First local command |
|---|---|---|---|---|
| `release_preflight.yml` | Manual release gate before tagging/publish. | Manual run on `master`. | Version parity checks, release secret presence checks, `devctl release-gates` (CodeRabbit triage + Ralph only; preflight self-check skipped), release/runtime/security/docs bundles, strict-warning hygiene mode, plan-index commit continuity, architecture-surface sync, workflow-shell hygiene, workflow action-pinning, bundle/workflow parity, platform-contract sync + closure guards, CodeRabbit gate checks, IDE/provider isolation, compat-matrix schema/smoke, naming consistency guards, function-duplication + structural-similarity guards, serde-compatibility guard, `cargo deny` + release security gate, ship dry-runs, and AI-guard commit-range scoping via `workflow_shell_bridge.py resolve-range`. | `python3 dev/scripts/devctl.py check --profile release` |
