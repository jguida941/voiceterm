...
All notable VoiceTerm changes are documented here.
Some historical entries reference internal tooling that has since been removed.
...
- Removed the portable governance/control-plane implementation, generated
  authority state, review packets, dashboards, and repo policy projections
  from the VoiceTerm repository. The pre-cleanup repository remains available
  through the `v1.2.5` tag and Git history.
- Removed the governance-only Dev panel, Action Center, developer event flags,
  operator console, mobile companion scaffold, and their dedicated tests and
  workflows. `Ctrl+D` once again passes EOF directly to the backend CLI.
- Removed the unreachable JSON-IPC runtime and its inert CLI flags, the
  uncalled legacy full-screen UI, diagnostic-only binaries, an unsupported
  Windows launcher placeholder, stale help entries, and unused launcher code.
  The supported daemon socket/WebSocket integration remains available.
- Renamed the remaining shared CLI worker and output-sanitization module from
  the misleading Codex-only name to provider-neutral `agent_runtime`.
- Replaced the governance command wrappers with focused `make` targets and
  standalone scripts for product checks, release notes, version validation,
  checksums, and Homebrew updates.
- Moved the product changelog and Markdown configuration to the repository
  root, and updated user, contributor, security, automation, and release docs
  to describe the standalone VoiceTerm project.
- Corrected source-run instructions so `make run` and `./scripts/start.sh`
  locate or download a Whisper model. Raw binary instructions now show the
  required `--whisper-model-path` when no checkout-local model is present.
- Corrected the control reference: while recording in `insert` mode, `Ctrl+E`
  finalizes early, transcribes, and stages text in the chat composer without
  sending it; pressing `Ctrl+R` again cancels and discards the capture.
- Clarified that Claude can defer transcripts while busy, while Codex stages
  them immediately in its visible composer.
- Standardized Cargo on whisper-rs's checked-in bindings so clean local, CI,
  release-binary, and Homebrew builds do not depend on host Clang layout
  generation.

### Preserved

- Voice capture, local Whisper transcription, Codex and Claude integration,
  HUD controls, themes, transcript history, macros, memory/privacy controls,
  capture-once mode, and the generic daemon integration interface.
- Rust tests, product regression scripts, CI, security checks, PyPI packaging,
  release binaries, and Homebrew publishing automation.

### Terminal fixes included

- Codex voice transcripts captured while Codex is thinking, responding, or
  running tools appear immediately in the visible composer instead of being
  hidden in a delivery queue.
- Left/Right traverse every Codex HUD button; Up/Down leave HUD focus and pass
  through to Codex composer and prompt-history navigation.
- Settings, transcript history, help, and Theme Studio open without repainting
  the conversation or forcing a top-to-bottom scroll flash.
- Theme Studio keeps an isolated live HUD preview visible beneath its controls;
  HUD style, border, right-panel, color, and glyph changes now repaint that
  preview immediately without touching the underlying conversation.
- JetBrains keeps the Codex HUD visible during active work and uses atomic,
  settled redraws to prevent detached borders, stale fragments, flicker, and
  artifacts during pane dragging or terminal resizing.
- Cursor passes Claude slash-menu Left/Right input through to Claude while
  preserving VoiceTerm HUD navigation behavior.

### Validation

- Full Rust formatting, Clippy, test, integration, performance, memory, and
  PTY parser checks pass on the standalone repository.
- PyPI packaging tests, Markdown/link checks, workflow linting, version parity,
  and release-helper checks pass.
...
- Clarify JetBrains + Claude as fully supported on current releases, while
  keeping a rare long-parallel-turn edge-case note (temporary HUD/transcript
  overlap at turn completion) with a clear workaround: resize the terminal once
  to force layout recalculation (`README.md`, `QUICK_START.md`,
